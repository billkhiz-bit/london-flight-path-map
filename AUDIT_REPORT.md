...
| A-0724-M4 | ~~`js/api-base.js` is pinned in the SW shell cache with a never-bumped VERSION — an API base rotation never reaches installed PWAs.~~ **CLOSED — fixed and verified live 2026-07-29.** `js/api-base.js` is now deliberately excluded from the shell precache (`sw.js:37`), `/js/` is served by `networkFirstAsset()` with `cache: 'no-cache'` (a plain SW `fetch()` still reads the HTTP cache, so CloudFront's TTL would have rebuilt the staleness) plus a `fresh.ok` guard, and `VERSION` is `v1.0.2`. Live `sw.js` is byte-identical to source (SHA-256 match), so this is deployed, not merely source-fixed. | `sw.js:29,37,155` |
...
2. ~~I1 (sw.js offline-shell), I2 (status.html), I6 (footer), I3 (NYC render), I7-I10 a11y~~ — **DONE + deployed 2026-07-24**. A 2026-07-25 correction here recorded that **M4 was NOT fixed** and had been wrongly struck through: the sw.js work closed I1 only, leaving the cache-first coupling that strands installed PWAs on a stale `js/api-base.js`. **That correction is itself now spent — M4 was closed and verified live on 2026-07-29** (see its row above). It was recorded as gating the `api.skyscore.co.uk` CNAME swap on the grounds that landing it afterwards is too late for anyone already installed; **that gate is lifted, and the CNAME swap is free to proceed.**
