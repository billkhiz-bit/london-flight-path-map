...
| I6 | Leicester 0/8 and Teesside 0/5 carry **no** road-noise or flood band; three defences pass green | `data/borough-extra.json` | Verified. The map is honest; **CLAUDE.md was over-claiming, corrected today**. **CLOSED 2026-08-29, and the finding's PREMISE was wrong.** This was recorded as a property of the data by three documents. It was an unrun script: `fetch_defra_road_noise.py` and `fetch_ea_flood_risk.py` are both per-city against ENGLAND-WIDE coverages, both cities are in England, both have boundary files, and neither was in `NO_ROAD_COVERAGE`/`NO_FLOOD_COVERAGE` — the rasters had simply never been fetched for the two cities that joined on 2026-08-11. Leicester is now 8/8 on both; Teesside is 5/5 on road and 0/5 on flood, held by ONE near-all-sea tile that renders blank and that the C11 guard rightly refuses to cache. **A measurement recorded without its cause reads as a constraint.** |
