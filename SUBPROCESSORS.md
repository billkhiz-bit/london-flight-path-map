...
|---|---|---|---|---|---|
| 1 | **Amazon Web Services Inc.** (AWS) | Sole compute, storage, CDN and DNS-record-set host. Runs Lambda, API Gateway, DynamoDB, S3, CloudFront, and **Simple Email Service (SES)** - one plain-text confirmation email per `/v1/signup` request, sent from eu-west-2 (verification-first signup, OPERATIONS.md s3.9). | All API requests in transit; signup email at rest; pending signup requests (address, any display name, a random token) at rest for at most 24 hours; confirmation emails in transit; user favourites at rest. | eu-west-2 (London) for compute/state; CloudFront edge POPs globally for static assets only. | UK GDPR Art. 28(3) DPA via the [AWS Service Terms](https://aws.amazon.com/service-terms/) and [AWS DPA](https://aws.amazon.com/compliance/gdpr-center/). |
