...
|---|---|---|---|---|---|
| 1 | **Amazon Web Services Inc.** (AWS) | Sole compute, storage, CDN and DNS-record-set host. Runs Lambda, API Gateway, DynamoDB, S3, CloudFront, and **Simple Email Service (SES) - NOT YET SENDING** (built 2026-09-15 behind `SignupVerify=off`; when on, one plain-text confirmation email per `/v1/signup` request, eu-west-2, see OPERATIONS.md s3.9). | All API requests in transit; signup email at rest; user favourites at rest. | eu-west-2 (London) for compute/state; CloudFront edge POPs globally for static assets only. | UK GDPR Art. 28(3) DPA via the [AWS Service Terms](https://aws.amazon.com/service-terms/) and [AWS DPA](https://aws.amazon.com/compliance/gdpr-center/). |
