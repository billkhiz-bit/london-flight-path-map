...
| Native iOS / Android distribution | **In-flight 2026-05-10 (Wave 13.8.5)**: full release pipeline now wired. Capacitor wrapper + Codemagic CI for iOS; Android local-build via Android Studio per `mobile/ANDROID_BUILD.md`. fastlane lanes operational on Windows; first `bundle exec fastlane ios metadata_only` succeeded against the live `uk.co.skyscore.app` ASC record — description, keywords, copyright, privacy URL, support URL, marketing URL, app review notes all auto-push from `mobile/fastlane/metadata/ios/`. Bundle ID `uk.co.skyscore.app` registered at developer.apple.com; ASC app record "Sky Score" created. ASC API key + Issuer ID stored in `.env`. `privacy.html` deployed to S3 (`/privacy/index.html`, CloudFront fronted). Remaining: trigger first Codemagic iOS build (manual dashboard click), upload screenshots from a TestFlight install, run `fastlane ios submit_for_review`. | Once iOS submission lands in App Store review queue (target: this week) |
