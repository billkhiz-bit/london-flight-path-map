...
| **Gate `/v1/regions` and `/v1/changes`** (audit I1) | **Not a template change, and not safe as one.** Two PUBLIC pages call these with no key: `changes.html` (`/v1/changes`) and `score-demo/status.html` (`/v1/regions`), and `tests/area-page-freshness.mjs` and `scripts/score_bulk.py` read `/v1/regions`. A public page cannot hold a key - the same argument that keeps `/v1/environment` open. Safe only after a decision on the PAGES: either `/changes` bakes its data at build time (the area-page pattern), or the routes stay open and I1 is closed as won't-fix with that reason. | If gating: (1) rewrite `changes.html` and `status.html` to not call the route, or serve them from build-time data; (2) give the freshness gate and the bulk scorer `SKY_SCORE_API_KEY` on that call; (3) extend `ScoreDemoUsagePlan` if the demo key must reach `/v1/regions` - `tests/demo-key-scope.mjs` asserts the demo key reaches ONLY `/v1/score` and will red, correctly, so change the test's expectation in the same commit; (4) SAM deploy; (5) `curl` both routes without a key expecting 403. | `/changes` renders with its explanations; `score-demo/status.html` shows every route green; `demo-key-scope.mjs` green with its new expectation. | A public page goes blank with no error in the console the user reads - the shape of every silent failure in this repo. Redeploy the template with the gate removed. |
| **`OPTIONS /v1/environment` returns 403** | Any time Bill says so - it LOOSENS auth on an OPTIONS method that returns headers only, no data. Nothing triggers a CORS preflight today; the first caller adding a custom header will. Bundle it with the next SAM deploy rather than deploying for it alone. | Remove `ApiKeyRequired` from the OPTIONS method in `template.yaml` (its siblings `/v1/signup` and `/v1/chat` are already open); SAM deploy. | `curl -X OPTIONS -H 'Origin: https://example.com' -H 'Access-Control-Request-Method: GET' .../v1/environment` -> 204 with the CORS headers. | None that matters - reverting is the same one-line change. |
| **Rename `healthcareWithin1kmPct`** (it measures 500 m; audit I2) | **Any time - and it is smaller than the audit recorded.** Measured 2026-09-10: the key lives in `data/borough-extra.json` only (a public URL, hence "public field"), written by `build_borough_bands.py`; `/v1/score` does not emit it, `/v1/environment` does not, `openapi.yaml` does not name it, and `index.html` does not read it by name - only three `design/` prototypes and `design/hydrate.js` do. No integrator contract, no notice period. | Rename in `build_borough_bands.py` (`DERIVED_KEYS` and the writer), run `--write` so the holder follows, update the four `design/` references, rebuild area pages if the label appears there (it does not today). One commit. | `build_borough_bands.py --check` green (the check compares its own key list against the holder, so a half-rename reds), `grep -rn healthcareWithin1kmPct` returns only history. | A stale key left in the holder beside the new one - the check catches it as a holder-only field. |
| **CPI / real-terms growth adjustment** | After a decision AND a costing like v5.0's: measure the movement on every borough before writing it. It touches `growth`, which only the `investor` persona weights, so `balanced` scores do not move - but the trend field is published to everyone. Needs a source with a cadence (ONS CPIH, monthly) and a place in the vintage-roll sequence. | Prototype the formula in a script that prints per-borough before/after; write METHODOLOGY s4 first; then engine + site together (`borough-score-parity.mjs` is the OUTPUT gate); version bump + 14 days' notice. | `check_worked_example.py` (the s6 example carries `trend`), parity on 91 boroughs, `/v1/changes` explains the movement. | A formula in one holder and not the other - the Manchester-incident shape. The parity gate exists for this. |
| **IAM privilege-escalation path** (OPERATIONS s3.8, audit C3) | **Only with Bill at the console, at the START of a working session, with no load or deploy running and time to test a real deploy straight after.** The 3 Sep console edit REPLACED the policy instead of extending it and no deploy of any kind could run for a day; nothing detected it until `check_aws_permissions.py` was written. | `python scripts/check_aws_permissions.py` BEFORE (expect 18 granted, 0 denied); make the edit with the whole of `backend/iam-policy.json` open beside it; run the probe again; run a real `sam deploy` of an unchanged stack; run `scripts/check_deploy_drift.sh`. | 18/18 both times; the deploy completes; drift 133/133. | Paste the whole of `backend/iam-policy.json` back - that is what fixed 4 Sep. A permission is a timestamp, not a property: re-probe, never read it off a document. |
