...
**Last reviewed:** 2026-08-10 (**EIGHT MORE CITY-REGIONS ON THE API, THREE ON THE SITE.** `/v1/score` now serves 11 cities; the consumer site still serves 3. Every field of the eight is script-derived and verified against the publishing body - HPI prices, ONS crime, DfE Progress 8, ONS-coded boundaries - with aircraft bands an explicit runway-geometry ESTIMATE. **The DEFRA blocker never existed**: Round 4 publishes an Lden surface for all 16 English airports including every one of ours, and the provenance sentence claiming otherwise was false and is fixed. **Next, in order:** six cities onto the website (needs the boundary loader to stop being a per-city if/else chain), then un-gate postcode resolution (one `if city != 'london'` plus the NSPL borough attribute - but resample corridors to ~1 km first or every new city reads systematically QUIETER than London), then sample the DEFRA rasters per airport. **Unchanged and still the actual bottleneck: 5 outreach drafts, 0 sent since 21 May, and 6 warm Build Night connections unfollowed. skyscore.co.uk has no DKIM and no DMARC, so cold email would land in spam; warm follow-ups from Gmail need neither.**)
