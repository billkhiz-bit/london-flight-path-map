...
| Native iOS / Android distribution | **In-flight 2026-05-09 (Wave 13)**: Capacitor wrapper at `mobile/` + Codemagic CI for iOS/Android binaries. PWA shipped first (manifest + service worker + install chip + iOS Add-to-Home-Screen hint). Native scaffolding ready; awaiting user-side dashboard config (Codemagic ASC integration, Android keystore, Play Console service account). Apple Section 4.2 defence is the GPS-based "Score where I am" feature. Store listings + privacy policy + Apple review notes drafted in `mobile/`. | Once first Codemagic builds succeed and apps land in TestFlight + Play Console internal track |
