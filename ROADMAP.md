...
**Last reviewed:** 2026-05-16 (Wave 13.17 — CERT_PRIVATE_KEY persistence shipped. Build 17 was never triggered after the 13.16 push; rather than do the one-off cert revoke-and-retry dance, the `codemagic.yaml` fetch-signing-files step now decodes a persisted RSA private key from a Codemagic Secure env var (`CERT_PRIVATE_KEY`, group `asc`). Every build sees the same public key, so ASC matches and reuses the existing Distribution cert instead of creating a new one — Apple Personal Account's 2-cert ceiling stops biting permanently. Memory + `mobile/CODEMAGIC_SETUP.md` section 3 document the one-off local key gen + dashboard paste + cert revocation. Once Bill completes those manual steps and pushes, the first persisted build will create cert #1 and every subsequent build reuses it. Icon PNG fix from 13.16 is unchanged and ready to ship in the new build. Pre-13.17 state in `HANDOFF_2026_05_14_icon_ship.md`.)
