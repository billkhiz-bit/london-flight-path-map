...
**Last reviewed:** 2026-05-16 EOD (Wave 13.18.1 — **v1.0.1 submitted to Apple, awaiting verdict**. Build 19 is the first successful build under the persisted-cert pipeline: orange radar icon visible in ASC, attached to v1.0.1, submitted for review. Cert situation permanently resolved: one Distribution cert created from the persisted `$CERT_PRIVATE_KEY` (PEM, public-key SHA256 `c680ad18...805934`), every future build reuses it instead of orphaning a new cert. The arc from 13.16 → 13.18.1 took three failed builds to land: 13.17 attempted base64-in-env-var (corrupted in clip.exe→textarea transit), 13.18 switched to raw PEM (dashboard Edit on a Secure var didn't persist the value due to a Codemagic UI quirk), 13.18.1 added a length-diagnostic and the user delete-and-recreated the env var. Eight distinct failure modes resolved across the icon-ship arc; full forensic trail in `HANDOFF_2026_05_14_icon_ship.md` and `memory/feedback_codemagic_personal_account_signing.md`. Apple review window 24–72h; in-flight at submission.)
