...
> **UPDATE 2026-05-18 EOD — iOS resubmitted as build 20.** Apple flagged Guideline 4.0 (Design) on iPad Air 11" M3 on 2026-05-18 — UI "crowded" on iPad. Wave 13.20 (commit `64cea5b`) added iPad-landscape + iPad-portrait CSS breakpoints, narrowed sidebar at iPad widths, and stopped the bottom sheet from auto-opening on iPad portrait. Codemagic built build 20, fastlane uploaded to TestFlight, build 20 resubmitted to Apple from App Store Connect (~EOD 2026-05-18). **Next session: check ASC for Apple's verdict on build 20 BEFORE doing anything else on iOS.** Android can still proceed independently for the Play side below — but rebuild the AAB first (`npm run build:android`) so it carries the iPad fix, otherwise Android tablet users see the same cramping Apple flagged. The on-disk AAB at `mobile/android/app/build/outputs/bundle/release/app-release.aab` is stale relative to master. See [[HANDOFF_2026_05_18_build20]] for the next-session pickup checklist.
