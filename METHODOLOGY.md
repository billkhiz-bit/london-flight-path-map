...
Where a component has no source, the response says so rather than publishing a silent default. Greater Manchester currently returns `sourceBreakdown.live` beginning **"NO DATA SOURCED"**, because no schools, crime, transport or healthcare input exists for that city yet and the published liveability value is a placeholder rather than a measurement. Its `quiet` band is likewise declared a **provisional estimate** from Manchester Airport runway geometry, not a DEFRA raster sample — the dB Lden thresholds in §3 are not evidenced for that city.
