...
> Version 5.3, last updated 2026-09-26.
...
**The overlay's legend is the publisher's own scale (corrected 2026-09-25).** The overlay renders DEFRA's WMS style unchanged: ten 5 dB intervals from 40 dB (<=40 drawn white at 0.5 opacity, which the layer's multiply blend makes invisible, so nine are visible), in teal, green, peach and plum. Until 2026-09-25 the legend beside it showed five bands from 55 dB in a different palette, so the pale green 45-55 dB halo - the largest area on the London map - read as 60-64 dB. The legend now reproduces DEFRA's ColorMap (and, for New York, the BTS layer's own seven bands from 45 dB) and `scripts/check_noise_legend.py` holds it to both the served image and the publishers' live styles. The 55 dB reporting threshold above is unchanged: it governs what is SCORED, while the overlay shows everything DEFRA draws.
...
### 4.3 Growth, dual-anchor scale on the real-terms trend, against the national pool (v5.2)

**Since v5.1 (2026-09-15) the trend growth is scored on is the REAL-terms 12-month change**, the nominal HM Land Registry HPI change deflated by ONS CPIH for the same month:

```
trendReal = ((1 + trend/100) / (1 + CPIH/100) − 1) × 100     rounded once to 1dp, then used as the input
```

so that the 5.0 anchor means *prices held their value against inflation*, not *did not fall in cash terms*. `context.priceTrendPct` stays the nominal figure — it is a published field integrators read — and `context.priceTrendRealPct` and `context.inflationPct` sit beside it, so the score can be reproduced from the response. **Why:** costed before deciding (`scripts/cost_real_growth.py`), at CPIH 2.8% for June 2026, **25 of the 77 boroughs the product called "rising" were falling in real terms** — a borough at +2.2% nominal was losing value, and the panel said "rising". The form is the exact ratio, not `trend − CPIH`; the difference is a few hundredths of a point at these rates and the exact form costs nothing. **New York is NOT deflated**: no US inflation series is held, and dividing a dollar trend by a UK index would be a number nobody measured, so its growth stays nominal and `context.growthBasis` says `nominal` (sterling cities say `real`). The deflator for each vintage lives in `CPIH_12M_PCT` in the engine, one entry per vintage, because `?compare=previous` recomputes the previous score under the current formula and deflating May's trend by June's inflation would manufacture movement.

Growth is then a **dual-anchor** scale on that trend. A flat market — prices holding their real value — sits at the midpoint, and each tail is scaled to its own extreme **across the whole currency pool** (since v5.2, 2026-09-16; within the city until then):

```
growth = 5 + (trend / max_trend) × 5    where trend > 0   (max_trend = fastest real-terms riser in the pool)
growth = 5                              where trend = 0
growth = 5 − (trend / min_trend) × 5    where trend < 0   (min_trend = steepest real-terms faller in the pool)
```

all then clamped to 0–10, with `trend` read as the real-terms figure from v5.1. The fastest-rising borough **in the pool** scores 10, the steepest-falling scores 0, and a market holding its real value scores exactly 5. The pool is every borough sharing the currency — the 94 sterling boroughs across all twelve UK city-regions, or New York's five for USD — the same pool §4.2 uses for affordability, for the same reason.

**Why the pool and not the city (v5.2).** With each city scaled to its own extremes, **one borough per rising city scored 10.0 and one per falling city 0.0 every month, by construction** — 22 of 94 boroughs on a rail whatever their trend, and a city's leader changing hands was a full-scale flip. Replayed over 24 months of HPI (2024-08 to 2026-07, all 94 sterling boroughs, deflated month by month), the mean month-on-month change in a borough's growth score was **1.44 points, 8.2% of transitions moved five points or more, and the mean within-city spread was 8.79 of 10**. The July 2026 roll made it visible: Hartlepool's trend went +0.8% → +5.5% and its growth **0.0 → 10.0** because Teesside's leader changed hands; Newport 8.5 → 0.0 for the same reason in Cardiff's four. Pooling the 94 real trends **halves the churn (0.76 mean; 0.2% moving five or more)** and leaves one borough at each rail. Smoothing the input instead — a three-month mean — was measured and rejected: it barely touches the churn (0.95) because the per-city rails remain, and it would publish a trend that no longer equals HMLR's own `Annual_Change`, which is what an integrator verifies against the source. The within-city standing the old scale implied is published beside the score as `context.growthRankInCity` (`{rank, of}`, 1 = fastest real-terms riser of the city), exactly as `priceRankInCity` did for affordability at v5.0. Growth is therefore **comparable between cities**: a borough rising 3% in real terms scores the same in Teesside as in London.

**Why the midpoint is absolute and the tails are relative.** The 5.0 anchor is a real-world fact (0% price movement), so it does not drift when the pool is re-based at a refresh. The extremes are pool-relative, which keeps the scale usable when the whole market moves. This is the same relative reasoning applied to both directions rather than only upward.

**Why the tails are scaled separately.** The London cohort's trends ran from −28.1% to +3.1% as of the 2026-Q2 snapshot (the sterling pool's real trends run −23.1% to +6.3% at July 2026). A single symmetric map across that range would compress every rising borough into the top fifth of the scale, making +3.1% nearly indistinguishable from +0.4%. Scaling each side to its own extreme keeps both tails legible.
...
For the London cohort as of the 2026-Q2 snapshot, `max_trend` was +3.1% (Waltham Forest) and `min_trend` −28.1% (City of London); **twenty** boroughs carried negative 12-month trends. For the sterling pool at July 2026 (v5.2), `max_trend` is +6.3% real (Trafford) and `min_trend` −23.1% real (Westminster); the London comparison on `/v1/changes` names those yardsticks, qualifying one outside London with its city.
...
**Why not absolute thresholds?** UK property markets are cyclical; absolute growth thresholds would need re-calibration every market cycle. Pool-relative scaling captures *relative momentum across the covered market*, which is more durable as a signal.
...
**Flight-path geometry (v5.3, 2026-09-26): derived from the UK AIP.** London's corridors are generated by `scripts/build_flight_paths.py` from the UK Aeronautical Information Publication (AIRAC 09/2026), and the procedure behind each one is recorded in `data/flight-procedures.json`:

- **Final approaches**, every runway end at Heathrow and London City: the extended centreline from the published threshold (AD 2.12) out to where an aircraft on the published ILS glide path (AD 2.19) passes 3,000 ft. Heathrow's 3.0° gives 9.4 NM, between the AIP's own 2,500 ft daytime and 10 NM night joining rules; London City's 5.5° gives 5.1 NM.
- **Departures**, cut at 20 km along track: Heathrow's twelve Noise Preferential Routes geometrised from the wording of EGLL AD 2.21 paragraph 8 (magnetic radials corrected by the AIP's published 1.24°E variation), and London City's RNAV departures from their SID coding tables, which list every waypoint's coordinates in the order flown.
- **The segment from a holding stack to the final approach is not drawn.** EGLL AD 2.22 says it is "flown under direction from the Radar Controller": there is no published track, so any line would be invented.

Before v5.3 the corridors were hand-drawn, and four Heathrow "stack" lines ran from each hold straight to the runway - Ockham and Bovingdon due north-south into two east-west runways - while the departure lines fanned off the runway end where the published routes run straight ahead first. A member of the public pointed this out ("look up the SIDs and arrival charts... nobody turns that early"), and they were right. Gatwick, Luton and Stansted carry no London corridors, as before; the other UK cities keep their runway-centreline corridors until each airport is taken through the same AIP derivation. New York's eight corridors are unchanged: the UK AIP does not cover them. Each corridor is a sequence of waypoints at 1 km; the shortest distance to any waypoint is the proxy for distance to the corridor.

**The corridor penalty is fitted, not assumed (v5.3).** The corridor ladder (4/3/2/1 points at under 1/2/4/6 effective km) is scaled by `CORRIDOR_WEIGHT = 0.3`. `scripts/fit_corridor_weight.py` scores every DEFRA-measured postcode in every city with a loaded raster (26,233) through the geometry tier, picks the weight that minimises mean absolute error on a train half split by a hash of the postcode, and reports the other half. At the old full weight the airport term and the corridor term both penalised the same postcodes near every runway, and London's estimate read a mean 1.8 points louder than DEFRA measured. Held-out results:

| City (test half) | Postcodes | Error at weight 1.0 | Error at 0.3 | Signed bias at 0.3 |
|---|---|---|---|---|
| London | 9,402 | 2.273 | **1.277** | −0.15 |
| Greater Manchester | 1,089 | 2.238 | **2.047** | +0.25 |
| West Midlands | 701 | 2.407 | **1.707** | −0.32 |
| Bristol | 630 | 2.329 | **1.258** | −0.91 |
| Merseyside | 526 | 3.208 | **2.045** | −2.01 |
| Leicester | 232 | 3.376 | **2.546** | −1.93 |
| West Yorkshire | 218 | 2.668 | **1.389** | −0.67 |
| Nottingham | 212 | 2.862 | **2.040** | −0.81 |
| All | 13,115 | 2.359 | **1.431** | −0.29 |

Every city with at least 150 test postcodes improves; Tyne and Wear (105) is too thin to report on its own. **The yardstick has a blind spot, and it decided how it is used**: DEFRA maps only the loud strip around each airport, so it rewards corridors that hug the runway axis and cannot see a departure route where it turns out of that strip. Measured, the AIP geometry at full weight scored *worse* than the hand-drawn lines (2.16 against 1.97 in London). So DEFRA sets the SIZE of the penalty and never the geometry, which comes from the published procedures.
...
A real end-to-end calculation at **methodology v5.1**, using `SW11 1AA`
(Battersea, in the London Borough of Wandsworth). Every figure below was taken
from the live API on 2026-09-07 and re-derived from the engine; the growth
working and the persona table were re-derived for v5.1 on 2026-09-15.
...
Then the dual anchor against the STERLING POOL's REAL-terms extremes (§4.3,
methodology v5.2 - every one of the 94 boroughs in the pool, not the London
cohort): `min = -23.1` (Westminster), `max = 6.3` (Trafford, Greater
Manchester); Wandsworth is falling, so it is scaled against the steepest fall:
...
Westminster is both London's and the pool's steepest faller, so for a falling
London borough v5.2 changes nothing in this arithmetic; a RISING London borough
is now scaled against Trafford's +6.3% rather than Barking and Dagenham's
+2.1%, and a rising Teesside borough against the same +6.3% rather than its own
city's leader - which is the point (§4.3). The within-city standing is
published beside the score as `context.growthRankInCity`, 26 of 33 for
Wandsworth.

(Nominal, this would be 3.7 from −5.5% against a nominal pool of −20.7 to
+9.6. The 0.5 the borough loses is inflation. At the June vintage, under the
London cohort, the real figure was 3.6, from −7.8% against −27.4 to +1.5.)
...
persona-borough combinations, the worst residual is **under 0.08** (0.0721 as
...
> This table was stale from v5.0 (2026-09-09) until 2026-09-15: it still carried the pre-v5.0 within-city affordability of 6.5, so every row was about 1.7 too high while the block above it said 5.0. Re-derived from the engine's own rounding with the five components above; it is not yet gated.
...
| AIP corridors, fitted weight (v5.3) | **1.320** | **−0.146** |
...
7. **HM Land Registry, UK House Price Index (HPI)**, the official monthly UK property price index, used as the source for the Affordability and Growth components: <https://www.gov.uk/government/collections/uk-house-price-index-reports>
   7a. **ONS, CPIH annual rate, all items (L55O, MM23)**, the deflator for the Growth component since v5.1: <https://www.ons.gov.uk/economy/inflationandpriceindices/timeseries/l55o/mm23>
...
- **2026-09-26 (v5.3)**, **Flight-path corridors derived from the UK AIP; the corridor penalty fitted against DEFRA.** (1) *Defect:* London's corridors were hand-drawn. Four Heathrow lines named after the holding stacks ran from each hold straight to the runway - Ockham and Bovingdon due north-south into two east-west runways - although arrivals are radar-vectored from the holds onto a straight-in final, and the departure lines fanned off the runway end where the published routes run straight ahead before turning. Raised by a member of the public on 2026-09-25 ("look up the SIDs and arrival charts... nobody turns that early"); correct. (2) *Measured before deciding:* the corridor term lowered quiet for **48% of London's non-DEFRA postcodes**, by a mean 2.2 points where it applied (about 0.7 on the `balanced` headline). Against the DEFRA-measured postcodes, **removing corridors entirely improved the estimate in 6 of 7 testable cities** (London 1.97 → 1.54), and swapping in the correct AIP geometry at full weight made it *worse* (2.16), because the airport-distance and corridor terms double-count near every runway. The weight, not the geometry, drove the error. (3) *Change:* §4.5's corridors are generated from the UK AIP (AIRAC 09/2026) by `scripts/build_flight_paths.py` into both holders - every runway end's final approach to 3,000 ft on its published glide path, Heathrow's twelve Noise Preferential Routes from their AD 2.21 wording, London City's RNAV departures from their SID coding tables - and the corridor ladder is scaled by `CORRIDOR_WEIGHT = 0.3`, fitted on a train half of 26,233 DEFRA-measured postcodes across eight cities by `scripts/fit_corridor_weight.py`. (4) *Effect:* held out, the geometry tier's error against DEFRA falls **2.359 → 1.431** overall and in every city with enough postcodes to report; London's full-set figure, published on the API page, falls **1.879 → 1.320** with the signed bias **−1.742 → −0.146** (still on the pessimistic side). Postcodes DEFRA measured are scored from the raster and do not move; borough bands and borough scores do not move, being derived from runway centrelines separately (`build_aircraft_bands.py --check`, 114 of 114 unchanged). What moves is `quiet` on geometry-tier postcodes near a corridor, mostly UP, and the site's neighbourhood ranking, which scores on the same geometry. (5) *Guard:* `build_flight_paths.py --check` is blocking in preflight and reds if either holder is hand-edited away from `data/flight-procedures.json`; `test_corridor_weight_matches_the_site` compares the weight across the Lambda and index.html and asserts both site ramps READ it (proven red at 0.5 against 0.3); `fit_corridor_weight.py --check` is advisory (NSPL is gitignored) and reds if the shipped weight stops being the fit; `check_quiet_estimate_error.py`'s published-figure ceiling is tightened 2.2 → 1.5. (6) *Scope:* London only for geometry. The other UK cities keep their runway-centreline corridors, now at the fitted weight, until each airport is taken through the same AIP derivation; New York is outside the UK AIP and unchanged.
- **2026-09-16 (v5.2)**, **`growth` is anchored on the currency pool, not the city cohort.** (1) *Defect:* the dual anchor (§4.3) scaled each city's rising tail against ITS OWN fastest real-terms riser and its falling tail against its own steepest faller, so by construction one borough per rising city scored 10.0 and one per falling city 0.0 every month - 22 of 94 boroughs on a rail whatever their trend - and a 4-5 borough cohort's leader changing hands was a full-scale flip. The July 2026 roll made it visible the morning it landed: Hartlepool's nominal trend went +0.8% → +5.5% and its growth **0.0 → 10.0**; Newport +5.0% → +2.0% and 8.5 → 0.0; 78 of 99 `investor` scores moved on one ordinary monthly release. (2) *Measured before deciding:* the growth score was replayed month by month over 24 months of HPI (2024-08 to 2026-07, all 94 sterling boroughs, each month deflated by its own CPIH) under four designs. City cohort: mean month-on-month change **1.44**, 8.2% of transitions moving ≥5 points, within-city spread 8.79/10. National pool: **0.76**, 0.2%, 3.64. Three-month smoothed input on the city cohort: 0.95, 4.1% - the rails stay, and it publishes a trend that no longer equals HMLR's `Annual_Change`. Both together: 0.51, but with the smoothing objection. Same reasoning the product accepted for affordability at v5.0: *a narrow cohort manufactures spread it has not measured.* (3) *Change:* `max_trend`/`min_trend` are taken over every borough sharing the currency (`national_trend_bounds`, mirrored in `index.html`; the two API-only cities enter the site's pool through `BACKEND_ONLY_TRENDS`, maintained by `build_hpi_prices.py --write` beside their prices). The 5.0 = flat anchor is unchanged. `context.growthRankInCity` (`{rank, of}`, 1 = fastest real-terms riser of the city) publishes the within-city standing, as `priceRankInCity` did at v5.0. The comparison yardsticks (`benchmarks`, `/v1/changes` `marketContext`, the `why` workings) are the pool's, and one outside the borough's own city is labelled with it - "Trafford (Greater Manchester)". New York's USD pool is its own five boroughs, so nothing changes there. (4) *Effect, measured against v5.1 on the July data:* **`balanced` does not move** (growth weight 0.00); **`investor` moves on 56 of 99**, mean +0.11, largest Newport 4.2 → 6.0 and North Somerset 7.1 → 5.6; growth components at 10.0 go from 12 to 2, at 0.0 from 11 to 1; **cross-city inversions - a borough rising faster in real terms scoring LOWER growth than one rising slower - go from 651 of 4,371 pairs to 0.** The worked example (§6) is unchanged at growth 3.2, because Westminster is both London's and the pool's steepest faller; the published worst-case reproduction residual moved 0.0674 → 0.0721 and both documents now say "under 0.08". (5) *Guard:* `check_worked_example.py` derives the bounds through `national_trend_bounds` (it built a London-only list until now, which would have kept agreeing with a §6 quoting London's extremes after the engine stopped using them); `check_openapi_matches_engine.py` caught the new context key before the spec had it; `tests/borough-score-parity.mjs` agrees on 182 borough/persona pairs; `test_growth_is_anchored_on_the_pool_not_the_city` constructs a Teesside borough with a London borough's trend and asserts equal growth, and bounds the number of boroughs on either rail. Two tests that had borrowed London's fastest riser as "the" benchmark now construct their cohorts, because nationally that role belongs to a city with no previous vintage. (6) *Notice:* the signups register was read on the day - four rows, none a third party's key - so this entry is the record, as for v5.0 and v5.1.
