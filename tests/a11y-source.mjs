...
//
// IT JOINED WCAG_TAGS ON 2026-09-09, SO WCAG 2.2 NOW BLOCKS. The two steps were
// kept a day apart on purpose: running the rules first turned "audit the
// product for 2.2" into a MEASUREMENT, and the measurement was 1 rule, 16
// nodes, 1 state - `target-size` on the vendored Swagger UI page and nowhere
// else in 134 scanned page-states. Promoting an unknown backlog to blocking
// would have been the scope change this comment warns against; promoting a
// known and now-empty one is not. The advisory tally that sized it is deleted
// rather than kept - a mechanism whose only job was to answer a question that
// has been answered is the `.layer-transport` dead-rule shape.
