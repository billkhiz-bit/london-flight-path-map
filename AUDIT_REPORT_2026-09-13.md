...
| ~~M7~~ **PARTLY FIXED 2026-09-14 - SECURITY.md and the preflight skill no longer state a fixed advisory count; `ci.yml` gains `permissions: contents: read`. Dependabot, branch protection and SHA pinning are repo settings - a decision row in ROADMAP** | `npm audit`: 5 advisories (2 high), all dev-only, all `fixAvailable`; SECURITY.md says "4 high". Dependabot security updates disabled; `master` unprotected; `ci.yml` has no `permissions:`; actions pinned by tag | `package.json`; repo settings | Security [A] |
