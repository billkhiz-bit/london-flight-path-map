...
**Still required (user-side, not committable)**: Codemagic dashboard config (Apple ASC integration, Android keystore upload, Play Console service account JSON), App Store Connect listing creation, Play Console listing creation, real Apple Team ID + keystore fingerprint to replace the `.well-known/` placeholders, deploy `privacy.html` to S3 + CloudFront, first Codemagic builds.
