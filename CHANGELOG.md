...
**Still required (user-side, not committable)**: Codemagic dashboard config (Apple ASC integration only; no Android secrets), App Store Connect listing creation, Play Console listing creation, real Apple Team ID + keystore fingerprint to replace the `.well-known/` placeholders, deploy `privacy.html` to S3 + CloudFront, first iOS Codemagic build, first Android Studio local build.
