...
- **Analytics scope**: GoatCounter's `count.js` loads on the marketing pages AND on `/score-demo/` - the page that mints and displays a free-tier API key. It is a rolling third-party script with no SRI, inside the page's CSP. Recorded rather than removed on 2026-09-14 (audit M6): the demo's funnel events are the only measure of B2B interest, and taking them off is a decision for Bill (ROADMAP, Open decisions). The key it can see is a FREE-tier key (10,000 requests a month, `/v1/score` only), never a paid one.
...
- **Bedrock cost-abuse prevention**: all five Bedrock Lambdas (chat, multi_agent, analyze_image, analyze_document, report) and their API Gateway routes were removed on 2026-05-07 after a smoke test found them anonymously invokable. **`chat` was restored on 2026-08-06** as a retrieval-only assistant and is the only one back; the other four live in git history alone. The abuse controls that made restoring it acceptable: `POST /v1/chat` is **API-key gated**, is **denied to every self-service key** (`RateLimit: 0` on `ScoreFreeUsagePlan`, verified live), and the model never supplies data - context comes from invoking the score function directly and `verify_answer()` discards any reply containing a number absent from that payload. That control has fired in production. Corrected 2026-09-07: this bullet said the routes were "removed entirely" for a month after one came back.
- **Input never reaches a URL path unvalidated** (added 2026-09-07 after an audit). `lookup_postcode` - the single funnel both resolution tiers pass through - rejects anything but `^[A-Z0-9]{1,8}$` before the value is interpolated into a postcodes.io URL. `urllib.parse.quote` defaults to `safe='/'` and `.` is always safe, so `../outcodes/SW11` previously survived into a PATH segment, reached a differently-shaped endpoint and produced an unauthenticated **500 with a JSON body from `/badge`** - the one response that endpoint must never give, since it renders inside an `<img>` on a third-party listing page. Bounded SSRF (host-locked, no scheme or host control) rather than an open one. `str.isalnum()` is deliberately NOT used: it is true for Cyrillic letters and Arabic-Indic digits, so it answers a question about characters when the question is about ASCII. Regression test asserts both directions.
- **The device token is generated from `crypto.getRandomValues`**, not `Math.random` (changed 2026-09-07). That token IS the DynamoDB partition key for `/favourites`, so it is the only authorisation that store has - and the previous fallback's own comment defended it as "weaker entropy but still passes the Lambda's format check", which is true and is the wrong test. V8's `Math.random` is xorshift128+, whose state is recoverable from a short run of outputs. 128 bits now, against `randomUUID`'s 122, and `getRandomValues` exists in every browser that lacks `randomUUID` - including the non-secure contexts where that branch is actually reached.
...
- **Zero third-party code ships to the browser from npm.** `package.json` has an *empty* `dependencies` block — every npm package is a `devDependency` (ESLint, Prettier, stylelint, html-validate, Playwright), and the site is a single hand-written `index.html` with no build step, so nothing from `node_modules` reaches production. Verified 2026-07-27: `npm audit --omit=dev` reports **0 vulnerabilities**, while the full dev tree carries a changing handful of advisories in the linting toolchain (5 on 2026-09-14, 2 of them high; this sentence said "4 high" for weeks - audit M7 - and the number is `npm audit`'s to report, not this page's). Those affect developer machines only and are reported as advisory by `/preflight`. An earlier version of this line claimed a flat "`npm audit` clean (0 vulnerabilities)", which stopped being true as the dev tree aged.
...
- **The native build tooling under `mobile/` is a separate dependency tree, and it is where every open Dependabot alert lives (measured 2026-09-18: all 26 were in `mobile/package-lock.json` and `mobile/Gemfile.lock`; the root tree had one moderate, dev-only).** None of it ships to a user: it is the Capacitor CLI, the icon generator and fastlane, run on the machine that builds the apps. `npm audit fix` on 2026-09-18 took the npm side from 11 to 7 by pruning orphaned sub-trees and lifting `brace-expansion`, `minimatch` and `tmp`; **the remaining 7 (incl. the `tar` critical and `sharp`) all sit inside `@capacitor/assets@3.0.5`, the latest release, which pins `sharp 0.32.6` exactly and a nested v5 Capacitor CLI.** They are accepted as an upstream-pinned dev-tool residual, revisited at each native release: an `overrides` bump of a pinned `sharp` would fail on the next `build:assets`, unobserved until a release. The `Gemfile.lock` side (`faraday`, `jwt`, `excon`, `json`) is fastlane's and is updated with the next native release, where the pipeline it feeds gets exercised.
...
> **`.well-known/security.txt` points here as of 2026-09-07.** Its RFC 9116
> `Policy:` field had pointed at `LICENSING.md`, so a researcher following
> the machine-readable pointer was shown a data-source licence table instead
> of the safe-harbour terms and triage timeline below - which is exactly what
> that field is opened to find, and usually before deciding whether to test
> anything at all.
