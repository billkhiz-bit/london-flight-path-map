...
- **Log retention is currently unlimited — corrected 2026-07-26; remediation open and blocked.** This section previously claimed "default 90-day retention"; that was wrong. Log groups created implicitly by Lambda have **no** retention policy, and all 13 groups in this account (7 active + 6 orphaned from removed functions) read `retentionInDays: none`, i.e. *Never Expire* — re-verified 2026-07-29. **Planned remediation**, tracked in `ROADMAP.md`: delete the 6 orphaned groups and set 30-day retention on the 7 active ones. Both are console actions — the deploy user is deliberately granted neither `logs:PutRetentionPolicy` nor `logs:DeleteLogGroup` — and both were **blocked from 2026-07-27 to 2026-07-30 on an AWS Support case for root console access**. **That block is now cleared** — console access was regained on 2026-07-30 and a TOTP authenticator registered. The remediation is **scheduled, not yet performed**: as at 2026-07-30 a probe confirms `flightmap-dev` is still denied `logs:PutRetentionPolicy` and all 13 groups still read *Never Expire*, because the widened deploy policy in `backend/iam-policy.json` has not yet been applied. This bullet will be updated when the groups are actually changed and re-verified from the API; it will not be quietly removed. **Scope of the exposure:** the signup Lambda logged raw email addresses between 2026-06-26 and 2026-07-23, when the code path was fixed. Those entries sit in the `SignupFunction` log group, reachable only with authenticated access to the AWS account, and the remediation deletes that group outright rather than ageing it out — under GDPR storage limitation those are different remedies and only deletion actually removes the data.
