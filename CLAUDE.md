...
3. **Native iOS / Android** — Capacitor wrap at `mobile/`, built by Codemagic, distributed via App Store + Play Store.
...
mobile/
  ...
  scripts/render-assets.mjs # Playwright SVG→PNG for capacitor-assets
  assets/                   # icon source SVGs (icon, foreground, background, splash)
  CODEMAGIC_SETUP.md        # one-off dashboard config (ASC API key, keystore)
...
codemagic.yaml              # repo root; ios-workflow + android-workflow
```

**Update cadence:** web changes (CSS, JS, copy) deploy to CloudFront immediately. Native binaries need a Codemagic build + store review (~2-3 days Apple, ~1 day Google). Plan binary releases every 2–4 weeks at most — more often than that isn't worth the review-cycle cost.

**Local dev (Android only — iOS needs a Mac which we don't have):**
```bash
cd mobile
npm install                  # one-off
npm run sync                 # rebuilds mobile/www/, syncs Capacitor
npx cap open android         # Android Studio (must be installed)
```

iOS native project is regenerated by Codemagic's `ios-workflow` on each cloud build — `npx cap add ios` runs in the cloud Mac, not locally.
