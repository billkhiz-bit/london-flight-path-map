...
- **Use `/aws-debug` when Lambda errors or API Gateway 5xx issues occur — IT WORKS (re-measured 2026-09-02).** `logs:FilterLogEvents`, `logs:GetLogEvents` and `logs:DescribeLogStreams` are ALLOWED, proven by reading production output. **This line said the opposite from 2026-07-26 to 2026-09-02**, and the cause is worth knowing: the probe that "verified" the denial used the log-group name written down HERE (`.../ScoreFunction-LuxoNSLxJMva`), and **a Lambda's log-group suffix changes whenever CloudFormation REPLACES the function** - the live one is `.../ScoreFunction-AQH1Sxwg3LaF`. A missing group returns `ResourceNotFoundException`, and a probe treating any non-zero exit as "denied" turns that into a permissions wall. **Resolve the name from `describe-log-groups`; never hardcode a suffix, here or in a script.** What IS still denied: `cloudwatch:DescribeAlarms`, `cloudwatch:ListMetrics`/`GetMetricData` and `lambda:ListFunctions` — so logs are readable but **metrics and alarms are not**, which is the difference between "why did this request fail" and "is something failing right now". The `Observability` statement in `backend/iam-policy.json` covers exactly those and needs applying in the console. See `OPERATIONS.md` §6. Prefix any `/aws/lambda/...` CLI argument with `export MSYS_NO_PATHCONV=1` or Git Bash mangles it.
