...
Available commands:
  ...
  /aws-debug Debug Lambda/API Gateway issues (log read restored 2026-09-04)
...
> near-field floor compared the borough ring against it as though DEFRA's 55 dB
> Lden contour were a circle centred on the airport. **Round 4 contours are
> strips along the runway centreline** - East Midlands' is 21.2 x 3.8 km against
> a disc of r = 3.46 km - so the test was wrong in both directions at once.
>
> **RUSHCLIFFE missed the disc by 180 m** and published `Quiet skies 10.0/10`,
> band `low`, over **10.43 km2 at or above 55 dB peaking at 65.6 dB** - the
> fourth-largest aircraft footprint in the product. Solihull, at a comparable
> 9.21 km2, published `moderate-high`. Not a threshold: an inconsistency.
>
> **The floor now tests the borough against the >=55 dB cells in that airport's
> own GeoTIFF.** The measurement is CHECKED IN
> (`data/aircraft-footprint.json`, 6.6 KB, un-ignored in `.gitignore`)
> because `build_aircraft_bands.py --check` is BLOCKING and the rasters are
> 2.6-29 MB and gitignored - the same shape as `district-msoa-names.json`.
> `scripts/measure_aircraft_footprint.py --verify` re-derives it from the
> GeoTIFFs, which is what stops the two drifting. **The builder REFUSES to fall
> back to the disc if the file is missing**, because a silent fallback would
> reinstate the defect on every fresh clone.
>
> **Effect: 3 bands moved.** Rushcliffe `low -> moderate` (balanced **5.0 ->
> 3.4**, quietlife 6.4 -> 4.0, both exactly as the audit predicted by hand),
> North Tyneside `low -> moderate`, and **Knowsley `moderate -> low`** - the one
> optimistic move, checked rather than accepted: its boundary is **0.76 km
> clear** of the nearest >=55 dB cell.
>
> **Teesside (MME) and Cardiff (CWL) keep the disc**, and the data file says so
> explicitly. They are **not mapped by Round 4 at all**, so there is no contour
> to test against - "no contour published" is not "not measured here", and an
> absent key and a measured zero are the same shape in JSON.

| **environment** (v4.0, 2026-08-29) | `build_borough_bands.py --check` / `--write --write-lambda` | **Air quality 0.45 + road noise 0.35 + flood 0.20, from `airQualityWhoRatio`, `roadNoiseAboveWhoPct` and `floodMediumOrHighPct` - the CONTINUOUS fields, never the three-band map summaries** (68.1% share the modal air band, 54.9% the modal road band). **Road noise reads the SHARE over WHO 53 dB, never `roadNoiseLdenMedian`** - the median carries 41 distinct values to the share's 69 over an IQR of 1.7 dB, and ramping it 53->63 dB clamps 19 of 73 boroughs to a perfect 10. All three anchored on PUBLISHED thresholds: WHO 2021 -> the UK legal NO2 limit; 0% -> 100% over WHO's 53 dB Lden road guideline; 0% -> the EA's 10% Medium-or-High cut. **SCORING inputs, so they live in BOTH holders** - `test_borough_data_parity.py` covers all three, plus a `NAME_ALIASES` drift guard. **Use `--write --write-lambda`, not `--sync-lambda`**: sync copies from `borough-extra.json`, which SKIPS backend-only Cardiff and Nottingham, so Nottingham would silently miss road. 90/99 carry road; **90 `measured`, 0 `partial`, 9 `unavailable` (NYC 5, Cardiff 4)** - measured 2026-09-11 through `env_resolution()`; the `partial` tier has been unreachable since the flood georeferencing fix, as the note 260 lines below already said. |
| **BOTH OF THE ABOVE ARE OVER LIVE POSTCODES ONLY SINCE 2026-09-01 (audit F38)** | `build_borough_bands.py` | NSPL keeps TERMINATED postcodes with real coordinates and they were being counted - **915,867 of them, 38.7% of the scan**. The bands were weighted toward demolished estates and redeveloped industrial land. The `(99.999, 0.0)` guard catches only the UNLOCATABLE ones, which is why this looked handled; four sibling scripts already read `doterm`. The reader now HARD-FAILS if the column is absent, or if a scan that kept rows found no terminated ones. Same pass, **audit I3**: DEFRA road `0.0` means "surveyed, below the lowest mapped band" and was being dropped from the SHARE's own denominator, so `roadNoiseAboveWhoPct` was computed among the noisier postcodes only; `sample_raster` returns `(mapped, surveyed, inside)` now and the median alone stays over mapped readings. Its flood sibling twenty lines below had explained the identical trap all along. **Together: 811 fields updated across both holders.** |
...
| crimeRate | `refresh_crime_from_ons.py --check --all` | ONS Table C4, 0 differ; **blocking preflight stage since 2026-08-24, with a PER-CITY floor** (it used to pass having compared zero). **`--write --all` writes every holder city since 2026-09-13** - it was London-only, and for the eight cities it never reached the panel printed TWO false sentences from the absent fields: "Offence breakdown not published for this area" (audit I5; Table C4 carries it for every CSP row) on 53 boroughs, and on the 43 with no `crimeVintage` a DISCLAIMER of ONS - "Sky Score estimate - ONS publishes no recorded-crime rate for this area" - on figures that are ONS's and that this very gate verifies against ONS. Measured live on Birmingham. `crimeTop` is now 85 of 91 (NYC and the City of London are the honest absences), `vsLondonMedian` divides by LONDON's per-offence medians for every city so the label stays true, and no rate and no London record moved. The Oct roll is `--check --all`, then `--write --all`, then mirror any moved rate into the Lambda registry (`test_borough_data_parity.py` reds until you do). |
...
| impact | `build_aircraft_bands.py --check` | geometry ESTIMATE, ladder scaled by DEFRA footprint. **The near-field floor is a CONTOUR TEST since 2026-09-01, not a disc** - see the block below. Its measurement lives in **`data/aircraft-footprint.json`** and NOT in `<something>-boroughs.json`: **that suffix is a CONVENTION five scripts glob to enumerate the cities** (`check_flood_georef`, `fetch_ea_flood_risk`, `fetch_defra_road_noise`, `fit_city_projection`, `build_city_stations`), and the original name invented a phantom city called "aircraft-footprint" that the blocking flood gate caught the same day. Do not name a non-city data file `*-boroughs.json` |
...
**Neighbourhood medians are HM Land Registry PPD CATEGORY A ONLY since 2026-09-01 (audit C2).** Category B is HMLR's additional class - repossessions, power-of-sale transfers, buy-to-lets identified by mortgage, and every property-type-'O' row - and **HMLR's own median statistics and the UK HPI use category A alone**. So while both classes were kept, one product published two price bases: a borough `avgPrice` on A, validated against HPI by a blocking gate, and a neighbourhood `price` on A+B in the same panel. **412 of 485 published prices were wrong** - TS26 Hartlepool by 40% (125k -> 175k), WV2 by 26.7%, LS2 by -18.5%. **485 rows became 481**: four districts (L2, L28, SR1, B7) fell below the 30-sale floor once the repossessions stopped propping them up, which is the floor working. `--check` re-derives every published median from the PPD with a **per-city 95% share floor**; it is ADVISORY because the 155 MB PPD cache is gitignored, and it reports INCONCLUSIVE rather than PASS when the file is absent. `--check-names` now scopes corroboration to the districts a build will actually PUBLISH - a curated label for a dropped district is dead config, not a false claim.

**Neighbourhood ranking data.** London and NYC hold *curated* medians and a hand-assigned `crime` modifier inline. **All nine UK city-regions' 481 entries are generated** (2026-08-11; was Greater Manchester's 85 alone) — `python scripts/build_city_neighbourhoods.py --write-index` rewrites `index.html` between each city's `<CITY>-NEIGHBOURHOODS:START/END` markers from HM Land Registry's **bulk** Price Paid CSV (the linked-data API returns **HTTP 200 with an empty list** for a district query, so it cannot be used) plus NSPL for coordinates. A "neighbourhood" is a **postcode district**, districts under 30 sales are dropped rather than estimated, and `crime` is 0 everywhere because sub-borough crime is not published at that geography. **Boroughs come from the Lambda's `LAD_TO_BOROUGH`, matched to Land Registry's own district spelling by normalisation** (`Westminster` → `CITY OF WESTMINSTER`); a borough matching nothing is reported loudly, because a silent miss reads as "this borough has no neighbourhoods". One PPD pass and one NSPL pass cover every city. Do not hand-edit inside the markers; re-run the script. The 155 MB PPD cache and the JSON by-product both land in gitignored `data/`.
...
"station list could not be loaded" on every search. **AND DATA-DEPLOY FIRST:
`python scripts/make.py data-deploy web-deploy`, never the reverse (corrected
2026-09-19).** `web-deploy` uploads AND INVALIDATES `index.html`, which carries
5 references to this file and 0 inline `_STATIONS` arrays, so web-first leaves
a window between the two invalidations in which the live page fetches a file
the origin does not have. Reversed there is NO window: the page that is live
carries the inverse and ignores the new file until the new page lands. **This
was live for a day and the runbook had it backwards in three holders** -
ROADMAP twice and the 18 Sep memory - **while the Makefile's own comment on
this very file had the reasoning right all along.** Mirror drift between a
comment and a runbook. Deployed and verified 2026-09-19: three hash-MATCHes,
`stations.json` **200 where it was 403** (it had never existed at the origin),
both invalidations Completed, drift **134 of 134** (18 data files, this is the
18th), Manchester renders Market Street and Piccadilly Gardens, London renders
Clapham Junction. **`web-deploy-all` now runs `data-deploy` before `web-deploy`
too (2026-09-25)**; `test_web_deploy_all_ships_data_before_the_page` in
`tests/test_make_runner.py` asserts it and is proven red on the old order.
...
Supertram names each direction as its own NaPTAN node and no strip in
`clean_name` touched them, so **Attercliffe published as five "stations"**
("Attercliffe", "... From City", "... To City", "... Platform to City",
"... Platform to Meadowhall"). Across the published arrays **170 of 943 entries
were a place already listed, 166 of them South Yorkshire** - which is how a
quarter of "nearest four stations" panels filled four rows with fewer than four
PLACES. South Yorkshire is **268 -> 102**. And NaPTAN keeps RETIRED nodes with
real names and coordinates, marked `Status: inactive` - **806 of them** were
published as current services, because nothing read the column.

**A THIRD instance turned up the moment the new gate ran**, which is this
repo's "hardening a gate finds a new defect" for the fourth time: Manchester
published **"Besses o'th'Barn" AND "Besses o'th'barn"** - one Metrolink stop,
two entries, differing only in a capital B. Same root cause as the directional
suffixes, a dictionary keyed on an exact display string, and the directional
strip could not touch it. The key is `casefold()`ed now and the DISPLAY name is
CHOSEN - the spelling with more capitals wins, deterministically, because this
array is diffed on every rebuild.

**The strip is anchored to the end, and was proven safe rather than reasoned
about**, because `clean_name`'s own docstring records a strip that EDITED a name
("Station Approach" -> "Approach"): of the 180 names it changes, **175 merge
into a place listed within 800 m**, and the 5 with no sibling keep a real place
name ("Meadowhall Interchange To City" -> "Meadowhall Interchange"). The reader
**hard-fails if the `Status` column is absent** and **again if a scan that kept
stations excluded no inactive ones** - the same two-directional guard the
terminated-postcode reader carries, because either spelling of a silent
fall-through looks like a clean run.

**11 entries left entirely and 4 of those are not closures - read this before
re-deriving.** Angel Road, North Woolwich, Silvertown, Oldham Werneth and
Garston (Merseyside) are genuinely closed. Three Eccles Metrolink ENTRANCE nodes
went while Eccles itself stayed. **Woodlands Road has no active node in NaPTAN
at all**, which is an upstream claim we follow rather than second-guess. And
**Grange Hill is a live Central line station that London now does not list**:
its active nodes sit at lon 0.09207, **outside** the boundary polygon, while the
retired node at 0.09276 fell inside - the London/Essex boundary runs through the
station, about 50 m apart. It was published only because a decommissioned node
landed on the right side of a line. The limitation that exposes is real and
PRE-DATES this fix: point-in-polygon containment never lists a station just
outside a city, however near the reader is standing. (This paragraph said they "draw the map's transport layer" until
...
count one measurement twice, and London's THEN 18 hand-picked interchanges meant
something different from "every station" (London's array is NaPTAN's ~670 rows
too since that same day; this line said "is 18" until 2026-09-15, audit M23).
**Since M23 (2026-09-15) a trailing QUALIFIER is split off and restored only
when unanimous**: "Abbey Wood" + "Abbey Wood (London)" and Hammersmith x4 (one per
line) merge, 1,415 -> 1,390; "Kensington (Olympia)" and "Hayes (Kent)" keep the
name every node carries. Two traps, both the bbox/first-match
...
Rank-to-price correlation, measured over the rendered rows and
**RE-MEASURED 2026-09-03**: **+0.55 London (was -0.23 on 2026-08-12),
-0.06 NYC, 0.65-0.89 every generated city**. So nine of eleven lists are largely
...
aircraft quiet while affordability is 27% of the score. **Nothing is
...
stale-string version could be written. **0.6 was chosen because nothing sat between -0.23 and 0.67, and that
justification has EXPIRED**: London is now +0.55, squarely inside the gap
that was said to be empty, and 0.05 from flipping the disclosure on its
128 neighbourhoods. Nothing is over-claimed today - the threshold is
measured, so London simply says nothing - but the constant is now
arbitrary in a way it was not when picked. The cause is `environment`
(v3.9): prime central London scores badly on NO2, so Mayfair (GBP 2.5M)
and Belgravia sank while Cheam (GBP 470k) leads. Re-measure before
touching it; do not reason from the old figures.

**The 281 curated area labels are CORROBORATED, not recalled (2026-08-12).** Until then only Greater Manchester had any, so **273 of 503 districts rendered under a repeated post town** — Birmingham ×35, Liverpool ×29, Leeds ×16, Sheffield ×15. Nothing was false (the outward code is always beside the label) but a ranked list of thirty-five "Birmingham" rows says nothing. `NAME_OVERRIDES_BY_CITY` now covers all nine cities, and **`--check-names` asserts every label against that district's own House of Commons Library MSOA name**, evidence checked in at `data/district-msoa-names.json` so the gate needs neither NSPL nor a network. Blocking in preflight. **273 → 5**, the five being Bath ×2 and Darlington ×3, left as post towns on purpose.
...
2026-09-17 (audit M1, closed as option A)**: the site's own distribution
carries a `/badge` behaviour with the API as a second origin, keyed on the
`postcode` query string and honouring that Cache-Control, applied and verified
by `scripts/cloudfront_badge_behaviour.py` (two postcodes, `Miss` then `Hit`,
different SVGs). Until then the cache was the BROWSER's only - `X-Cache: Miss`
on every call, measured 2026-09-14 - so every distinct viewer's first render
was an invocation. **Both badge URLs in index.html read ONE holder,
`BADGE_BASE = 'https://skyscore.co.uk/badge'`**, ABSOLUTE in both places
because the snippet is pasted onto other people's pages and the preview renders
inside the native app at `capacitor://localhost`; that host is in the CSP
`img-src` for the same reason. **`sw.js` passes `/badge` through**: same-origin
paths default to cache-first there, which would have pinned the first render in
Cache Storage where Cache-Control cannot reach it - the `borough-extra.json`
incident, on an image. `tests/test_badge_edge_cache.py` holds the four
together. Two things measured doing it: API Gateway has no HEAD on the route,
so `curl -I` answers 403 while a GET is a hit (browsers GET images); and
`SW11+1AA` and `SW11%201AA` are two cache keys, so use `encodeURIComponent`
consistently. **The badge is now served from the SITE's origin**, so the
Lambda's XML escaping and its `[A-Z0-9]{1,8}` postcode filter guard
skyscore.co.uk itself, not only the pages that embed it - an SVG opened as
a document runs script, and this one answers on our hostname.
...
template cannot answer that. **Verify after every deploy.** **It is conclusive
in every quota state since 2026-09-13.** It used to report the two demo denies
UNPROVEN whenever the demo's 2,000/month was spent (11 Sep to 1 Oct, every
month the demo is popular), reasoning that a 429 is a 429. API Gateway says
WHICH 429 in the body, and evaluates the per-method throttle BEFORE the quota:
a RateLimit-0 route answers `{"message":"Too Many Requests"}` whatever the
quota holds, and a route with a real throttle falls through to
`{"message":"Limit Exceeded"}`. So `deny()` reads the body - and "Limit
Exceeded" on a route that should be denied is now a FAIL (the request passed
the method throttle), where before it was an exit-0 UNPROVEN. The free-tier
...
- **F41 - a Capacitor build shipped 2 of 13 cities' geometry.**
  ...
  (their union, **18 files since `data/stations.json` joined on 2026-09-18**;
  17 before) with a floor of 10, because a regex that matches nothing must not
  produce an empty requirement. Verified by running it: 4 -> 17, and every
  sw.js-precached file present. **Re-measured 2026-09-19: sw.js 11, index.html
  18, union 18** - the derivation picked `stations.json` up with no edit, which
...
- Run `/preflight` before every commit — or directly: **`sh scripts/preflight.sh`** (also `npm run preflight`, `make preflight`; all three invoke the same script so they cannot drift apart). Blocking: ESLint (now `.js`/`.mjs` too, not just `index.html`), html-validate, ruff over `backend/lambdas` + **`backend/tests/`** + `scripts/` + `tests/`, **both** pytest suites, **extension extraction + extension e2e + responsive (EVERY public page since 2026-08-22, not just the homepage - widening it found `privacy.html`, `changes.html` and `score-demo/status.html` all scrolling sideways on a phone, the worst of them moving the WINDOW 402px at five viewports off a `position: sticky` header escaping its scroll container; the homepage keeps all TWELVE viewports - ten portrait plus the two landscape ones added 2026-09-01, see the D3 note below - and static pages get the narrow end; now run against SOURCE as the blocking half and against CloudFront as an advisory one — the same split `a11y-source.mjs` makes, because pointed only at live it goes red on a tree that has already fixed the defect and stays red until deploy) + `every city switches`**, plus **`panel says what it measured`** (`tests/panel-caveat.mjs`, added 2026-08-29 - the first gate that ever RENDERED the borough detail panel; nothing had, which is how "undefined only here" reached production) and **`panel contrast, borough (AA)` / `panel contrast, area (AA)`** (`tests/panel-contrast.mjs`, written 2026-09-01 and wired in the same day - it MEASURES the effective contrast of every visible text node itself rather than asking axe, which returns **0 violations and 66 `colour-contrast` INCOMPLETE** over this state because it cannot resolve an effective background, and a gate reading `violations` counts every one of those as a pass. It found **46 nodes below AA**, not the 2 the audit named. It is TWO stages because the two states have different network needs: measured with every offsite request aborted, the borough panel measures 134 nodes at desktop and 87 at phone while BOTH area states fail with `opened=ranking row, score rows=0` - the area panel is reached by a ranking-row click that runs `triggerSearch()`, which resolves the district through `api.postcodes.io`. **That was also the signature of an intermittent red on a defect-free tree**, because the gate allowed a fixed 1200 ms settle; it polls for the state now, bounded at 15 s, so a state that never opens still reds and a slow third party does not) (added 2026-08-06; the responsive audit fails on horizontal overflow **and, since 2026-08-11, on a control past the viewport edge with no scrollable ancestor** — it had always BUILT that list and only PRINTED it when the page itself scrolled sideways, so the city chips clipped by the map container's `overflow: hidden` left it reading "ok" at all ten viewports while three of eight UK cities could not be tapped at 320px; tap-target findings stay advisory because they need judgement; `every city switches` is `tests/city-switch.mjs`, added the same day because nothing had ever clicked a city chip; the e2e loads the extension into a real Chromium and needs `--headless=new`, since Playwright's normal headless uses `chromium_headless_shell` which does not load extensions at all), API-URL drift, **score sanity against the live API** (`scripts/check_score_sanity.py` - **28 probes since 2026-08-12, one per city; it was 16 and ALL LONDON**, which is why postcode scoring could be broken for eleven cities for two days with every gate green. The new probes assert the RESOLVED CITY, not that a score came back, because a London default returns a well-formed error. They caught a live false provenance claim on their first run - Manchester still said "PARTIAL, 2 of 4 inputs measured" two days after v3.6 and v3.7 gave it all four. The only stage that can catch a DATA defect; the pytest suites never reach DynamoDB and Playwright asserts the site against itself), **no em dashes on the 9 deployed pages** (`terms.html` joined 2026-08-05), **self-hosted fonts on all 9** (`tests/fonts-selfhosted.mjs`, serves the repo locally so it validates source before a deploy), **log retention == `privacy.html`** (`scripts/check_log_retention.sh` — see below; it now parses the claim out of `privacy.html` and passes honestly), **WCAG over the SOURCE tree** (`tests/a11y-source.mjs`. **Since 2026-08-31 it scans 109 pages, not 9** - the 9 public pages at **three** viewports (desktop, phone, and phone LANDSCAPE, added 2026-09-07 for audit I11; this said two until 2026-09-11, as did the array's own comment) plus **all 100 under `area/`**, which were in no accessibility or responsive gate at all. The area list is DERIVED by walking `area/`, never written down, and runs at ONE viewport because the generator emits no width breakpoint - a fact the gate ASSERTS by reading `build_area_pages.py` and failing if one appears, rather than leaving it in a comment. Same day, `AXE_TAGS` gained `best-practice`: **`FAIL_MODERATE` had been unreachable since the day it was written**, because all four of its rules are `best-practice`-tagged and the builder asked for WCAG tags only, so axe never RAN them. It was hiding missing `<main>` landmarks on `privacy.html`, `terms.html` and every area page. `failing()` keeps the old bar for everything else, so adding the tag did not quietly promote ~30 more rules to blocking. **It still scans the app's LANDING state only** - the borough panel has never been scanned by anything, and the 31 Aug audit found four defects living there. Original note follows: added 2026-08-10; since 2026-08-22 it also scans the **expanded legend**, because on a phone the legend ships `aria-expanded="false"` and axe does not evaluate hidden elements - so adding a mobile viewport had NOT made the mobile legend reachable, and three headings sat at **1.00:1**, literally the same colour as their background, until measured. It found a second untouched defect on its first run. *Adding a viewport is not the same as reaching the state.* Original note follows: added 2026-08-10 — the Playwright a11y spec scans CloudFront, so an accessibility regression could not be caught until it was already serving users, which is exactly how the locator inset shipped `role="img"` around ten focusable markers; this one serves the repo on 8923 with the CloudFront extensionless rewrite reproduced, and gates the deploy where the e2e one catches a bad deploy), **flood == EA service (georef)** (`scripts/check_flood_georef.py --all --per-class 4`, blocking `net_check` since 2026-08-30 - the ONLY flood gate that crosses a source boundary. `build_borough_bands.py --check` re-derives from the same mosaic, so the two things it compares are the file and itself, and it reported agreement for the whole period the mosaics were mis-georeferenced. This asks the EA's own GetFeatureInfo what it publishes at a BNG coordinate. **Sampling is the gate**: the first version passed the known-bad London file 9 of 9, because the six interior tile blocks were byte-identical and only clipped edge tiles had moved - `spread_samples()` draws one point per grid cell, PERIPHERY FIRST. Proven red at 20-60% across seven cities and green at 100% on all eleven. Carries a `MIN_COMPARED` floor (a class that reached the service twice has not been tested, and reports INCONCLUSIVE, never ok) and a top-up retry, because the EA host throttles and a gate that reds on rate-limiting gets switched off), **crime == ONS Table C4** (`scripts/refresh_crime_from_ons.py --check --all`, blocking since 2026-08-24 - the last of the audit's named gates to gain a per-city floor; its workbook auto-fetches and caches like the HPI stage's, ~1.7s), **map fits its box** (`tests/map-fit.mjs`, blocking since 2026-08-24 - 90 city/viewport combinations including landscape; see the 2026-08-24 memory entry for why nothing else could see a map drawn outside its own SVG), **prices == HM Land Registry** (`scripts/build_hpi_prices.py --check --all`, added 2026-08-10 — the only gate that can catch a PARTIAL VINTAGE ROLL, and written because there was one: London's `avgPrice` matched HPI 2026-05 for all 33 boroughs while its `trend` matched **no** HPI month, under a `CITY_PROVENANCE` sentence telling customers it was HPI. Keyed on **ONS codes, not names** — name matching has failed here five times. `--write` corrects BOTH holders or neither), **degraded + offline fallbacks** (`tests/failure-path.mjs`, blocking since 2026-08-27 - a stalled network, an offline launch and a partial TfL outage. It had been in NO gate at all since it was written: the one file dedicated to "the fallback shipped untested" was itself untested. Pointed at SOURCE for the same reason `responsive` and `a11y` are. Two things were living in that gap - it had been DYING on Node 24 at check 10 of 19 (a route aborted after `unroute`, unhandled rejection, fatal) so it read as a failing gate rather than a crashed one, and its NYC check had been clicking a selector that stopped existing when the switcher went two-tier on 2026-08-11, reporting London's 33 boroughs through a bare `catch` as though the switch had run), **web/native layout split** (`tests/native-sim-render.mjs`, blocking since 2026-08-31 - it and `tests/live-mobile-verify.mjs` were in **NO RUNNER AT ALL**, the 3rd and 4th orphaned gates here after `failure-path`. Two of its three contexts asserted **nothing** - the native and desktop halves measured, printed and compared nothing, so the App Store layout could break in any way and it exited 0. Now proven red both ways against real index.html defects. `live-mobile-verify` is **advisory**, because it reads CloudFront and a source tree ahead of the last deploy is the normal state), and Playwright at `--workers=2`
  - **`log retention == privacy.html` is BLOCKING and now PASSES, honestly (2026-08-06).** It asserts that AWS matches **whatever `privacy.html` §2d claims** — it parses the figure out of the page rather than hardcoding one. Until 2026-08-06 it hardcoded `WANT_DAYS=30` and never opened `privacy.html` despite its name, so the only route to green was the console work, and switching §2d to the honest interim wording left the gate **red on a truthful tree**. `DRAFT_security_retention_passage.md` §2b had flagged exactly this. The rewrite is strictly stronger: it still reds on "page says 30, AWS says None" and additionally reds on the reverse, which the old one could not see. **Both failure directions are proven red, plus an unparseable-claim case.** §2d carries **Version A** ("retained for 30 days") and `check_log_retention.sh` exits 0 against it - 7 live log groups match the page (re-verified 2026-08-21). This sentence said Version B for some time after the page had moved on, which is why the GATE is the authority here and not this file. **The console work in §1 is DONE, and was already done before anyone checked (measured 2026-08-21).** The account holds **exactly 8 log groups, one per live Lambda, every one at 30-day retention, and ZERO orphans** - so the 6 orphaned groups recorded here, and the Signup one said to hold raw emails from 26 Jun-23 Jul 2026, no longer exist. They had aged out under the retention that was applied to them. `flightmap-dev` DOES hold `logs:PutRetentionPolicy` (it lacks only the delete verbs), which is what made the fix possible without a console session. **Re-measure a recorded blocker before working around it.** The historical note: those groups used to **WARN, not fail**, because under an "indefinite" claim they do not contradict the page and deleting them needs `logs:DeleteLogGroup`, which `flightmap-dev` lacks — blocking there would gate every commit in the repo on a console action. When the console work lands, flip §2d back to Version A and the same check validates it. Gotcha: the AWS CLI here emits **CRLF**, so `retention=None\r` never string-equals `None`; the script strips `\r` and that line is load-bearing. Advisory, all TWELVE of them (it said ELEVEN until 2026-09-03, EIGHT until 2026-09-02 and FOUR until 2026-09-01 - the warning two clauses down has now been proven right twice, so obey it, omitting the two data-dependent gates that had been advisory since August - the same "a list of mirrors that omits a mirror is worse than no list" trap this file records about `template.yaml`; **count the `advise` call sites, do not trust this sentence**): Prettier, npm audit, **`p8 == DfE KS4 2023/24`**, **`borough bands == sources`**, **`neighbourhood medians == PPD`** and **`aircraft footprint == DEFRA`** (the last two added 2026-09-01), and **`quiet estimate == DEFRA`** (`scripts/check_quiet_estimate_error.py`, added 2026-09-02). **That one exists because `api/index.html` now PUBLISHES the estimator's accuracy to B2B buyers.** `Quiet Skies` is the headline component and is a geometry ESTIMATE for the great majority of postcodes; its error had been measured ONCE during the v3.8 work and recorded only in this file's prose - a number derivable by nothing, which cannot be re-checked, cannot go stale detectably, and cannot be shown to an auditor as more than a claim. Re-derived over all **35,352** London postcodes DEFRA measured: **MAE 1.879**, median 1.5, p90 4.2, and a signed bias of **-1.74 meaning the estimate reads LOUDER than DEFRA** - pessimistic, which is the safe direction for a noise product. It reproduced the recorded 1.879 exactly. **Do not quote it as a global accuracy figure**: it describes postcodes NEAR AIRPORTS, the only population where DEFRA published anything to compare against. And do not attribute the whole bias to the estimator - Round 4 maps a COVID-suppressed year, so some share is real traffic reduction, and the split is not measurable from what we hold, and **`aws perms == iam-policy.json`** (`scripts/check_aws_permissions.py`, added 2026-09-03; **it found a live outage on its first run** - the console edit applying the Observability statements had REPLACED `FlightMapDeployPolicy` rather than extended it, denying S3, DynamoDB, CloudFormation, Lambda, CloudFront, API Gateway and all four log-read verbs, so **no deploy of any kind could run** and nothing detected it. A capability PROBE, not a policy diff, because reading the live policy is itself denied; it classifies on the ERROR CODE and treats `ResourceNotFoundException` as GRANTED, since authorisation precedes resource lookup - reading that error as a wall is exactly what produced a false "no log access" claim here for five weeks), plus **`deployed == source`**, **`site == /v1/score`**, **`responsive, live`** and **`live web == tabbed layout`**, which run only when `--skip-e2e` is off. **That list named NINE of the eleven** when it claimed eleven, omitting the last two - the same "a list of mirrors that omits a mirror" trap this sentence is itself warning about, one level down. Twelve `advise` call sites, counted 2026-09-03. The four data ones are advisory on their INPUT alone - PPD, the DEFRA GeoTIFFs, the KS4 bundle and NSPL are all gitignored and a fresh clone has none of them, so blocking would red on a file no command restores. Each reports what it compared and refuses to pass on zero.
  - **`tests/responsive.mjs` gained a BOROUGH-SELECTED page state on 2026-08-31**, and it found the result-close `x` **20% hit-testable at every width <=900px** with `elementFromPoint` at its centre returning the sticky search card - a real click did not dismiss, and a screenshot crop showed no glyph at all. Same lesson as the legend state below, one step on: every entry was audited before a result exists, so the COVERED detector was written and had never reached the state that needed it. 55 -> 65 combinations. It also gained an **`[inert]` exemption**, tested rather than named like the two beside it - `closest('[inert]')` is the browser's own answer to "can this be reached", and deliberately NOT `aria-hidden`, which hides from assistive tech while leaving the element clickable. **`tests/selector-widths.mjs` now asserts every city chip is INSIDE the strip at >=901px**, which responsive cannot do: a chip past a scroller's edge is a scroll case and it correctly exempts one. With a mouse there was no way to scroll at all - no scrollbar exists, a vertical wheel leaves `scrollLeft` at 0 - so **6 of 10 cities were unreachable at 901px and Greater Manchester was still off-strip at 1366x768**. The strip wraps at >=901px now; below that the swipeable strip is untouched.

**`tests/responsive.mjs` gained TWO LANDSCAPE VIEWPORTS on 2026-09-01 (audit
D3), 65 -> 71 combinations, and the homepage runs 12 viewports not 10.** A whole
orientation had never been audited, which is how **three separate mobile rules
came to be keyed on WIDTH while HEIGHT was the failing dimension** - the sheet
peek, the layers popover, and the legend cap. At 844x390 the legend showed
58.2px of 427px of content and the chip that would expand it was `display:
none`, gated on `max-width: 480px`.

**The fix that closed it is the SECOND one tried, and the first was recorded in
three documents as the remaining step.** A flat `max(160px, ...)` floor asks for
more height than a 390px-tall viewport has, and a BOTTOM-ANCHORED panel takes
the difference from whatever is above it - here `div.search-box`, the full-width
sticky card at y 54-138, which the legend's own toggle then sat 5px inside. The
cap is now the band that is actually clear, `calc(100dvh - 126px - 146px)` =
118px there. That also **deleted the `min-height: 380px` boundary**: the
constant existed only to keep the flat floor off the city chips, and a
band-derived cap puts the legend's top at 146px throughout the range, so it
cannot reach them. *A magic number whose job is to contain another magic number
dissolves when you compute the real one.*

  - **The report could not name the culprit, and that cost the day.** The
    covered-detector printed `tagName + id` and never the class; `search-box`
    has no id, so it read literally `covered by div` - beside a legend full of
    divs. It prints `div.search-box` now. **When a gate names what it caught,
    make it identifiable: the elements that do the covering are anonymous
    layout wrappers by nature.** And the prescribed fix (make the toggle
    opaque, raise it) was **already committed while the gate still read 1 of
    71** - run the prescription before believing it. Raising it would also have
    passed the gate by covering the primary search input; *a green gate is a
    floor, not the goal.*
  - **The harness carve-out died with the CSS constant it mirrored.** The prep
    floor had been dropped to `vp.h >= 380 ? 40 : 8` to match the design, which
    meant 568x320 passed at a **26px legend against 427px of content**. Uniform
    40px again, proven red at exactly that state. *An exemption that mirrors a
    design constant must go when it does, or it keeps accepting the collapse
    the constant used to justify.*
...
**`tests/responsive.mjs` gained THREE detectors on 2026-08-23, and every one found a live defect on its first run.** It had only ever asked two questions - is the page wider than the viewport, and is a control past the horizontal edge with nothing to scroll it back. Both are about POSITION. Now also: **COVERED** (a control inside the viewport with something painted on top - `elementFromPoint` at its centre, which is the question a finger asks), **CLIPPED ABOVE** (the vertical twin of stranded; a 711px legend rendering from y=-374 was neither overflow nor past the horizontal edge), and a **legend-open page state**, because every entry was audited in its landing state and on a phone the legend ships collapsed - so the clipped detector could not see the defect it was written for. Two exemptions are tested rather than named: a control parked outside its own scroller is a scroll case (the city chips), and an element that returns into view when focused is the skip-link pattern (**focus it with `transition: none`** - `.skip-link` transitions `top` over 0.15s, so a rect read immediately after `focus()` reports the position it is moving away from, which is how that exemption failed on its first run).
  ...
  - **The two new advisory stages both compare DEPLOYED state**, which is why they are advisory rather than blocking. `scripts/check_deploy_drift.sh` compares **134 surfaces** against what CloudFront serves - 16 pages, the **18 files `make data-deploy` uploads** (17 until `data/stations.json` joined on 2026-09-18; measured 134 of 134 on 2026-09-19), and the **100 area pages** - plus a presence pass over `sw.js`'s 20 `SHELL_ASSETS` (drift between commit and deploy is expected, so blocking it would go red on nearly every run). **It covered only the 16 pages until 2026-09-02**, and reported "5 of 16 surfaces differ" on a morning when **all 100 area pages and `borough-extra.json` also differed** - a number over the wrong denominator, which is worse than none because it reads as complete. `tests/area-page-freshness.mjs` did not cover the gap either: it reads the SOURCE off disk, so a rebuilt-but-never-deployed set passes it. The two new lists are **DERIVED** - the data list from the Makefile target that deploys it, the area list by walking `area/` - so neither can freeze the way `copy-web.mjs`'s hand-written list did (F41). Each pass carries its own floor (>=18, >=100 - the data floor was >=17 until 2026-09-19, one below reality, so it could no longer have noticed a dropped file) and all three were **proven red**. `tests/site-api-parity.mjs` compares the score the **live site renders** against what `/v1/score` returns — the only check that reads the *output* rather than the inputs, added after the site and API disagreed on 13% of London postcodes while every component matched. Promote either to `check` once it has a track record; both already exit non-zero only on a measured problem.
  ...
  - `--skip-e2e` skips **only the eight stages that need the network** - the Playwright suite, the extension e2e, `area pages match the live API`, `flood == EA service (georef)`, `panel contrast, area (AA)`, `changes page explains why` (added 2026-09-07, when that orphaned gate was wired in), and `result close survives a scroll` (added 2026-09-25, audit M-9: it resolves a postcode through postcodes.io), and `noise legend == publishers` (added 2026-09-25: the live half of `scripts/check_noise_legend.py`, comparing both noise scales against DEFRA's and BTS's own styles) (added 2026-09-01; its borough sibling is a plain `check` because the borough panel renders with every offsite request aborted and the area panel does not). (It said THREE and omitted the flood gate from 2026-08-30, when that gate was added as a `net_check`, until 2026-09-01 - this repo's own "a list of mirrors that omits a mirror is worse than no list" lesson, in the paragraph explaining the flag. **Count the `net_check` call sites in `scripts/preflight.sh`; do not trust this sentence.** Note also that `score sanity (live API)` is a plain `check`, so `--skip-e2e` does NOT skip it and a fully offline run still reds there.) **Until 2026-08-28 it was a block wrapper that swallowed FOURTEEN stages while printing SKIPPED for two**, then printed a bare `RESULT: PASS`. Eleven of the twelve it hid never touched the network at all: `local smoke`, `degraded + offline fallbacks`, `locator inset`, `selector tiers`, `every city switches`, `map fits its box`, `UK city panel`, `area pages carry real data`, `layers paint only real data`, `responsive, source` and `WCAG source scan` all serve the WORKING TREE, which is precisely why they exist - they gate the deploy. So the flag documented as "skip Playwright (hits the live site)" was really a way to get a green run by not looking, and it hid exactly the gate set that caught every defect in the tabbed-default flip. Now `net_check()` marks the three per stage, a skipped stage still prints its own line **in its own position** (a stage that vanishes from a report is indistinguishable from one that passed), and a run with anything skipped reports `RESULT: PASS (INCOMPLETE - network stages skipped)` and names them. Report went from 20 stages to 32. `--fix` auto-fixes what is auto-fixable.
...
- **`/aws-debug` WORKS - all four log verbs granted, verified 2026-09-04.** The cause of the 3 Sep outage is now ESTABLISHED, where that entry could only say it was not: **`FlightMapDeployPolicy` had been REPLACED by the three Observability statements rather than extended by them**, so every verb outside that block was denied. The whole of `backend/iam-policy.json` was pasted back on 2026-09-04 and `scripts/check_aws_permissions.py` reports **18 granted, 0 denied**; the blocking `log retention == privacy.html` gate is green on 8 live groups at 30 days with zero orphans. **`backend/iam-policy.json` is APPLIED, not aspirational** - but only across the 18 actions the probe can safely exercise. It says NOTHING about the other 92, which are destructive or stateful and deliberately unprobed. Two loaders still carry comments calling the file aspirational (`load_nspl.py:170`, `load_defra_raster.py:295`) and **those are now stale** - `dynamodb:BatchWriteItem` especially is worth re-testing before assuming an NSPL roll is still stuck on the ~129 rows/s path. **Three lessons outlived the outage, and are why this entry is kept rather than deleted:** (1) **a permission is a timestamp, not a property** - this line has now been wrong in BOTH directions inside four days, so run the probe rather than reading the sentence; (2) **a Lambda's log-group suffix changes whenever CloudFormation REPLACES the function**, a missing group answers `ResourceNotFoundException`, and a probe reading exit status alone turns that into a false permissions wall - resolve names, never hardcode a suffix, and classify on the ERROR CODE; (3) the hazard sat in prose for five weeks and **converting it to a check is what found the outage** - the probe caught it on its first run, and nothing else could, since `iam:ListAttachedUserPolicies` is itself denied. The mirror-drift note that used to sit here - the command list at the top of this file contradicting this bullet - is resolved: **both lines were corrected together.** See `OPERATIONS.md` section 6.
...
all of them - **count the targets below, do not trust a number in this sentence**; it said "15 surfaces" while omitting `area-deploy` and its 100 pages entirely:
...
**`make` is not on PATH in Git Bash here, and since 2026-09-18 that no longer
matters: `python scripts/make.py <target>` (or `npm run make -- <target>`)
runs any Makefile target without GNU Make.** It reads the Makefile itself, so
there is one holder and nothing to keep in step; it refuses constructs it does
not implement (`$(shell)`, `ifeq`, `include`) rather than guessing;
`--dry-run` prints the expanded commands; and `tests/test_make_runner.py`
parses the whole Makefile on every preflight. `deploy_hpi_roll.sh` calls it
for its web steps instead of carrying retyped recipes. Until that date the
manual commands below were what actually ran on this machine, which is how
`OPERATIONS.md` s2 came to upload `privacy.html` to a dead key. They stay as
the reference for what each target does.
...
- **DEFRA raster quarantine LIFTED 2026-08-06.** `RASTER_TIER_QUARANTINED = False`. The blocker was that `index.html` computed quiet from geometry while `/v1/score` would answer from the raster, diverging on the ~9% of London DEFRA measures. Closed by `data/aircraft-quiet-london.json` (35,352 postcodes, 461 KB), which ships the **computed quiet score, not decibels**, so neither side reimplements the ramp. **Regenerate with `python scripts/build_aircraft_quiet_dataset.py` (and `--regions`) whenever `lden_db_to_quiet` changes** - the file embeds a `ramp` FINGERPRINT derived from the Lambda's own function (audit M25, 2026-09-14: it used to embed `methodologyVersion`, held at `'3.6'` against a Lambda at `'5.0'`, which was the WRONG key - that version moves for reasons that never touch the ramp, so "matching" it would have refused 42,773 measured postcodes on every unrelated bump), the page refuses a mismatch, and `tests/test_aircraft_quiet_dataset.py` reds the build when either shipped file or `AIRCRAFT_QUIET_RAMP_FINGERPRINT` in `index.html` disagrees with the Lambda. The builder no longer mirrors the ramp; it imports it. Deploying the file without the Lambda, or vice versa, recreates the divergence.
...
- **Active Lambdas** (in `backend/lambdas/<name>/app.py`):
  ...
  - **The per-postcode road tier was LONDON-ONLY until 2026-09-10, and a DEFRA
    zero is a reading.** `roadLdenDb` had been loaded from the London mosaic
    alone - SW11 served 69.1 dB while M2 4NG served `None` with "not measured,
    or still being loaded" - although all eleven city mosaics had been on disk
    since August. Nothing recorded it as open; it surfaced from asking
    `/v1/environment` at a new postcode after the NSPL roll. **Measured before
    loading** (`scripts/probe_road_raster_coverage.py`, the road sibling of the
    aircraft probe): 100% of live postcodes in every English city inside their
    mosaic and surveyed, 549,336 readings. And **2.0% of them are DEFRA zeros**
    (11,281; 0.7% London to 5.4% Leicester) - ground DEFRA SURVEYED and found
    under its lowest mapped band, **40.00 dB in every mosaic, measured** - which
    `load_defra_raster.py` dropped with the sentinels (`raw < LDEN_MIN`), so a
    surveyed-QUIET postcode got no row and told its reader the noise had not been
    measured. Audit I3 (the borough share, fixed 2026-09-01) one tier down;
    small, and directional, which is the bad kind of small. **Road mode now
    writes the zero as a BOUND under its own attribute, `roadLdenBelowDb`**, after
    asserting the open raster bottoms out at 40.00, and the Lambda publishes it
    as **`roadNoiseBelowDb`** - never as a decibel figure, because nobody
    measured 40 dB there, they measured "less than" - with a notice that says
    *quiet, not missing*. The extension renders it as `< 40 dB Lden`. **The
    runbook is `scripts/load_road_rasters.sh`**, `--live-only`: `/v1/score`
    never reads `roadLdenDb` and `/v1/environment` reaches a postcode only by
    reverse geocode, which returns live ones, so the 61% of the scan that is
...
- **DynamoDB tables** (4, all PAY_PER_REQUEST, eu-west-2): `london-flight-map-favourites`, `london-flight-map-signups`, `london-flight-map-noise-raster` (DEFRA Lden samples, loader `scripts/load_defra_raster.py`), `london-flight-map-postcodes` (ONS NSPL index, loader `scripts/load_nspl.py` — **rolled to the August 2026 edition on 2026-09-09: 2,704,825 rows in 58 minutes on the BatchWriteItem path.** (This said "roll to the August 2026 edition when it ships" until 2026-09-11, two days after the roll completed and directly above a block headed "THE 2026-08 ROLL IS COMPLETE" - the bullet is what a session reads first.) Next: the November edition, ~mid-late Nov). Both loaders run locally, not in Lambda, and both tables are forward-compatible: the score Lambda works correctly when they are absent or empty and upgrades silently once data lands, so **loading never needs a second deploy**.

  > ## THE NSPL GEOGRAPHY COLUMNS CARRY A YEAR SUFFIX AND IT MOVES (2026-09-09)
  >
  > February 2026 publishes `lad25cd` / `ctry25cd` / `rgn25cd`; **August 2026
  > publishes `lad26cd` / `ctry26cd` / `rgn26cd`**, and the file went 36 columns
  > to 35. `load_nspl.py`'s docstring called the read columns *"stable across
  > NSPL editions"*. They are not. **The CODES are unchanged** - all 94
  > `LAD_TO_BOROUGH` entries appear in both editions and both hold 363 distinct
  > LADs - so only the header moved, and no borough mapping needed touching.
  >
  > **FOUR scripts read these columns, and only one survived the rename.**
  > `build_city_neighbourhoods.py` already resolved by prefix and exits if it
  > cannot; the other three were pinned to `lad25cd`. All resolve by prefix now
  > (`re.fullmatch(r'lad\d\dcd', c)`), copying that existing idiom rather than
  > inventing a second one. **Do not pin the suffix again** - it moves every
  > February.
  >
  > **`build_borough_bands.py` was the dangerous one, and it FAILED SILENTLY.**
  > The loader hard-fails on a schema change, but the bands builder read
  > `row.get('lad25cd', '')` - and `.get` with a default **returns `''` rather
  > than raising**. Every postcode would have failed to match a borough, an
  > empty map would have been returned, and the only symptom was a line reading
  > `0 live postcodes across 0 cities`. Its floor was `if seen and not retired`,
  > which **never fires when `seen` is 0**. This is the script that WRITES both
  > score holders. There is a `if not seen` floor now, and it names the column
  > it resolved. *A `.get` with a default is a silent skip wearing a guard's
  > clothes.*
  >
  > **The download is not one file.** The Geoportal zip (~185 MB, item
  > `9d8364ebae8b4439aa66cda440e54fc8`) carries BOTH a combined
  > `Data/NSPL_AUG_2026_UK.csv` (964 MB) and a `Data/multi_csv/` split of 182
  > per-area files. The loader wants the COMBINED one, saved as `data/nspl.csv`.
  > Fetch it from
  > `https://www.arcgis.com/sharing/rest/content/items/<itemId>/data`; find the
  > current itemId through the ArcGIS Hub API rather than scraping the page.

  **THE 2026-08 ROLL, MEASURED BEFORE LOADING (2026-09-09).** Feb -> Aug, on the
  fields this product uses: **+5,494 postcodes and 0 removed**; live postcodes
  1,807,729 -> **1,810,364 (+2,635)**; 4,236 newly terminated and 1,827
  un-terminated; **72,554 positions refined**, median move **20.5 m**, 83%
  within 100 m, p99 1.5 km; and **81 LAD reassignments touching a borough we
  score** (77 of them live), which is 0.004% of live postcodes. A routine
  currency roll, not a re-basing.

  **Compare COORDINATES, not the string.** A first pass comparing the lat/long
  text reported **597,960** changed, which would have implied a national
  re-grid; comparing as numbers gives 72,554. The difference is trailing-zero
  formatting, and the wrong number was the alarming one.

  **NSPL loader speed - `BatchWriteItem` IS GRANTED as of 2026-09-09, re-probed.** This entry said the grant was "in `backend/iam-policy.json` and not yet applied to the live policy", and that **a ~6 hour roll is the signal the grant never landed**. It landed: `batch-write-item` with an empty `--request-items` returns **`ValidationException`, not `AccessDeniedException`**, and authorisation precedes validation. So the next roll runs the fast `_flush_batch` path (25 items per call), not the ~129 rows/s per-item fallback that took 5.80 hours on 2026-07-26.

  The likeliest cause is the 2026-09-04 restore, which pasted the whole of `backend/iam-policy.json` back after the 3 Sep console edit had REPLACED it - the same event that fixed `/aws-debug`. **Re-probe before planning around either figure**: a permission is a timestamp, not a property, and this one has now been recorded wrong in both directions. `scripts/check_aws_permissions.py` deliberately does NOT cover `BatchWriteItem` (it is a stateful write), so probe it the way that probe does - send a deliberately invalid request and classify on the ERROR CODE.

  Verify a load with `get-item`, never `describe-table`'s `ItemCount` (it refreshes ~6-hourly and reads 0 throughout).

  **THE 2026-08 ROLL IS COMPLETE (table 2026-09-09, derived shares 2026-09-10).** The table held the 2026-02 edition (2,699,394 rows, measured live) and was rewritten from `NSPL_AUG_2026_UK.csv` in 58 minutes at the batch-write rate - 2,704,825 rows. `NSPL_VINTAGE` in `load_nspl.py` was moved to `'2026-08'` AFTER the run, deliberately - **it is metadata written into the checkpoint payload, not read from the file**, so it does not self-correct, and the loader refuses to resume a checkpoint whose vintage disagrees with the constant, so editing it mid-run forces a restart from row 0. The `__META__` row was stamped `2026-02` from the running process's memory for the same reason and re-stamped by hand. **The re-derivation that followed moved 240 numeric fields across both holders, none of the four scored shares by more than 0.5, six `Environment` components by 0.1, ONE headline score (Merton 6.0 -> 5.9) and ONE band** - Dudley road noise `moderate -> low` on an unrounded share crossing 50% under a printed `50.0` that did not move. Bands are cut on the unrounded share BY DESIGN (METHODOLOGY s7.1, same rule as s6's round-once); 4 of 258 published band/figure pairs sit on a boundary that way, three of them pre-dating the roll. Do not band from the rounded figure to "fix" it.

  **The load is an UPSERT and that is safe here only because nothing was removed.** August adds 5,494 postcodes and removes none, so no row is orphaned. A future edition that DROPS postcodes would leave them behind in the table, still resolving, with no pass that deletes them - check `REMOVED` in the delta before assuming otherwise.

  **The derived shares are NOT updated by the load.** `transportWithin800mPct`, `airQualityWhoRatio`, `roadNoiseAboveWhoPct` and `floodMediumOrHighPct` all come from a scan of `data/nspl.csv` by `build_borough_bands.py`, not from DynamoDB - so a load leaves them describing the PREVIOUS edition's geography until `--check` and then `--write --write-lambda` are re-run - which is what happened between 22:33 on 9 Sep and the morning of 10 Sep. Rolling the table and not the bands leaves the two describing different vintages, and nothing but the advisory `borough bands == sources` stage says so. **And the bands are not the only readers**: eight scripts read `nspl.csv`, and the per-postcode DEFRA tiers (aircraft table, air-quality table, the two client quiet datasets) and the neighbourhood centroids all sample at POSITIONS - after the August roll 1,520 new live postcodes had no air-quality row and 154 sat on an aircraft contour but scored from geometry, with NOTHING saying so. A roll is done when every reader of the file has run; `HANDOVER.md` s0 step 7 is the list.
  ...
  **Run long loads detached and unbuffered**, or the next death is as undiagnosable as the last two — neither left a log, and the cause had to be read out of `Get-WinEvent -ProviderName Microsoft-Windows-Kernel-Power`. Use PowerShell `Start-Process` with `python -u` and redirected output so the run outlives the session. Check liveness with `sh scripts/load_status.sh` (reads checkpoint *mtime*, not file existence); a stopped loader resumes by re-running the same command. **`Start-Process sh.exe` inherits a bare Windows PATH** - no `dirname`, `tee` or `date` - so a runbook dies in a second with the only trace in the stderr file; prepend `C:\Program Files\Git\usr\bin` to `$env:PATH` in the launching PowerShell first (2026-09-10).

  **EVERY BULK LOAD IN THIS REPO'S HISTORY RAN ~50x SLOWER THAN IT SHOULD HAVE, FIXED 2026-09-10.** Both DEFRA loaders write each batch of 25 through a 25-thread executor, and their docstrings said that gives throughput "comparable to BatchWriteItem". **boto3's default connection pool is 10**, so 15 threads found the pool full on every batch, urllib3 discarded their connections, and each call paid a fresh TLS handshake. Measured against the live table with the loaders' own client: **sequential 17 ms/call (~60/s); 25 threads on a pool of 10, 13/s; 25 threads on a pool of 25, 675/s.** The executor was making things FIVE TIMES SLOWER than no executor. That is the untold half of the 14 h and 18 h air-quality deaths (they had to be running long enough to meet a laptop sleep), the "~1 hour" estimates that never came true, and Nottingham's road tier taking 17 minutes when Teesside's took 37 seconds after the fix. `ddb_write.MAX_WORKERS` is now the ONE holder for the width, `make_client` sizes `max_pool_connections` to it, both loaders import it rather than writing `25`, and `tests/test_ddb_write.py` reads the pool off the CONSTRUCTED client and fails if it is narrower. **The docstring number was never measured** - `feedback-numbers-in-justifying-comments-expire`, except this one was wrong the day it was written.
...
- **AI Tinkerers London, 22 Sep 2026** (demo proposal submitted 2026-09-15): a technical demo, not a pitch - the retrieval-only `/v1/chat` grounding check catching an invented 2030 price, the two holes audit I11 found in the check, and the preflight gates; framed as "what survived from an 'AI-powered' hackathon build". Draft at `OneDrive/Desktop/ai-tinkerers-london-2026-09-22-proposal.txt`, rehearsal notes at its foot. **Rehearsed against the LIVE endpoint 2026-09-17**: `SKY_SCORE_API_KEY` (the CI key) reaches `/v1/chat`; the free-tier key is denied there. Measured, 3 runs each: of 8 leading questions **1 in 24 fired the check** (the 2030 forecast, once, on the YEAR echoed from the question) - Nova Lite declines on its own now, so a live "watch it invent a price" moment is NOT reliable; the deterministic demo is the unit tests red-then-green, and the CloudWatch tab shows today's catch. **The rehearsal found the THIRD I11 hole**: "what is the score out of 100?" -> "50 out of 100", `grounded: true`, because the answer-side equivalence stripped 50 to "5" and `score: 5.0` supplied it - "about 500,000 pounds" passed the same way. Fixed (decimals-only on both sides), proven red on exactly those two, **DEPLOYED 2026-09-17 15:51 on Bill's instruction and verified live** - the out-of-100 question now answers `grounded: false` every time (the model always computes 50, the check always refuses it: a TRUE answer discarded, which is the honest cost of a control), so it is the one leading question that fires deterministically on stage - and that finding IS the talk's thesis. Applying does not confirm a slot.
- **AI Pilled [Operator] #02, 23 Sep 2026** (waitlist + demo form submitted 2026-09-16): a demo-night about HOW you run with AI, not what you build - home-grown workflows only. Pitched the Sky Score build workflow: Claude Code behind a 45-blocking-check pre-commit gate, every gate proven red first, cross-session memory of past mistakes, and the hard line that it cannot deploy to production. Demo, if picked: the 16 Sep data-release morning (HPI roll, two defects, v5.2 measured before decided, the classifier refusing the deploy, a contrast failure the gates caught). Draft + rehearsal notes at `OneDrive/Desktop/ai-pilled-operator-2026-09-23-form.txt`. Event was FULL at submission; waitlist. **2026-09-17: the organiser replied** - apologised for the Monday delay, invited Sky Score to the next [builder] event, and asked whether Wednesday's demo is the product or how the company is run (the former belongs at [builder]). **Replied the same day: the latter** - the workflow - and accepted the [builder] invite as a SECOND slot for the product itself; reply at the foot of the form file. Bill's note for the operator demo: "45 checks" is a dull headline - lead with the data-release morning and the three catches; government data and AWS enter as the pipeline being run and the deploy boundary, not as subjects.
- **AI Showcase: Public Sector Edition (London AI Hub)** (questionnaire submitted 2026-09-16): public-sector angle is that every input is OGL government data - "the public sector already publishes everything Sky Score is made of; I just joined it up". Answered "growth-stage" (nearer of two boxes) and NO to focusing on AI implementation/governance. Traction stated honestly: live product, App Store, Nova prize, pilots in conversation, no paying customer; the 182 visits since March deliberately NOT quoted. Demo video: the Nova one is 3:10 against a 3:00 cap - whether a trimmed link went in is Bill's to confirm. Draft at `OneDrive/Desktop/ai-showcase-public-sector-form.txt`.
...
The pending row is consumed by `DeleteItem(ReturnValues=ALL_OLD)` - read and
...
`inflationPct`, `growthBasis` are new. **New York stays nominal** - no US
series - and `scored_trend()` falls back by construction. **Read before the
July HPI roll:** the roll must ADD the new month's CPIH to `CPIH_12M_PCT`
(`build_hpi_prices.py --check` reds without it and compares the value to ONS
L55O); the site holds ONE deflator, `SNAPSHOT_CPIH_PCT` in index.html, which
rolls with it - `tests/borough-score-parity.mjs` now compares `investor` as
well as `balanced` because balanced weights growth at 0.00 and could not see a
stale site deflator (proven: 52 investor rows red, 0 balanced). The previous
vintage is deflated by ITS month (`previous_dataset()` re-derives after the
overlay). `market_context` stays NOMINAL deliberately (it describes prices);
the comparison's step 3 works in real terms and says so. Rounding is
`math.floor(x*10+0.5)/10`, the engine's `round_1dp` rule, NOT Python's
`round` - the two differ on a .x5 tie and the value has a JS holder. The
growth provenance line is DERIVED (`_growth_breakdown_line`), replacing 13
literals; a constructed test cohort must set `trendReal` too, or the engine
...
shipped `env` on 2026-08-26. `score-demo/openapi.yaml` - the artefact
integrators generate clients from - contained the string `env` **zero times**
across `Components`, `Weights` and `sourceBreakdown`, so a strict client
flagged an undocumented field on every UK response outside Cardiff and NYC
and a lenient one dropped 0.14-0.18 of the score; its `quiet` description
denied that road noise is scored (false at v4.0, where road is 0.35 of
`env`); and `enum: [london, nyc]` appeared in **four** places against
THIRTEEN served cities. `api/index.html` said "four components" and its
sample carried pre-v3.9 weights and `methodologyVersion 3.8` under a caption
promising "field names as the live API emits them". The demo drew four bars
summing to **0.86**, so a prospect saw a headline they could not reproduce.

**The consumer half was closed as I34 on 1 September and the B2B half was
not** - mirror drift, the largest cluster in the 31 Aug audit.
`scripts/check_openapi_matches_engine.py` is now **blocking** and derives its
expectation from `app.PERSONAS` / `app.CITIES` / `app.METHODOLOGY_VERSION`
plus the keys `resolve_query` emits, so the spec cannot describe an engine
that does not exist. **It finds enums by RECURSIVE WALK keyed on contents,
never by known path** - the spec's own comment records that "three enums in
this spec listed personas and only one was complete" - and that paid on its
first run by catching a **fourth** city enum (`BatchRequest`) the hand fix
had missed. It also distinguishes REQUEST from RESPONSE persona enums, which
was the gate's own first defect: `custom` is a response value only, and
`persona=custom` measurably returns 200 while silently scoring `balanced`,
so advertising it as an input would document a parameter that does something
else. Proven red five ways including one-enum-short-with-three-complete.
**The area-page half is CLOSED too (2026-09-03).** `build_area_pages.py`
adds `Environment` and all 99 pages are rebuilt: **90 carry it, and the 9
...
See `AUDIT_REPORT_2026-09-13.md` (**last full audit 2026-09-13**; `AUDIT_REPORT.md` is a pointer plus the carried-forward table, and the 09-11 and 08-31 reports sit beside it) for the live list. **Every Critical from every audit to date is closed and deployed as of 13 Sep, and 23 of the 13 Sep report's 24 Importants were closed and deployed the same night** (four tiers, each preflighted, each red-proven, each verified from the origin; the two dead deploy workflows are deleted). **The one open Important is I17** - `/v1/signup` writes any address unverified - and it is a DECISION, with three options in ROADMAP's open-decisions table, not a fix waiting to be typed. **On 2026-09-14, 36 of the report's 37 Minors were closed or converted to decisions across five preflighted commits, all deployed and verified from the origin.** That sentence said "only M14's chat-400 remainder is a fix still to type" and was wrong by one: **M23 was the 37th, never struck in the table, and the 14 Sep brief omitted it** - the list-that-omits-a-member trap, in the list of remaining work. **Both were closed on 2026-09-15** (chat 502 via `retrieve_context`'s third return value; station qualifiers via `split_name`/`display_name`, 1,415 -> 1,390), committed and **DEPLOYED 2026-09-15 and verified from the origin**: SAM modified `ChatFunction` alone; `index.html` uploaded `no-cache` and invalidated (completed); live hash == source; 1,390 stations served with 0 duplicates; `check_deploy_drift.sh` 133 of 133; live `/v1/chat` answers 400 with the score API's wording for a bad postcode and 200 for SW11 1AA. **Next session starts at ROADMAP -> Open decisions -> "What each open decision entails"** and its closing paragraph "Where this stands after 17 Sep": the badge edge cache is APPLIED, verified and repointed (17 Sep, audit M1 closed); the I17 pages are PREPARED on branch `i17-flip-verification-on` with a gate that keeps them red until the TTL lands (OPERATIONS s3.9 step 4); what remains is I17 steps 1-3 (SES, sandbox exit, two IAM verbs - Bill's), the EPC token rotation, and the AWS Marketplace row (after the rebrand). **Those two wrong numbers are FIXED AND DEPLOYED (2026-09-02).** The aircraft near-field floor was a DISC compared against runway-shaped contours (Rushcliffe published `Quiet skies 10.0/10` over 10.43 km2 at >=55 dB; it now publishes **3.4 / `Quiet skies 5.0` / `Moderate`**, verified live), and the neighbourhood medians included HM Land Registry **Category B** transactions, which HMLR's own statistics exclude - 412 of 485 published prices were wrong, so the product carried two price bases. **Both were SHAPE errors, not drift**, which is why a full re-derivation reproduced `borough-extra.json` with 0 disagreements and every gate stayed green throughout - the lesson to keep, since the next one will also be invisible to a consistency check. The long-standing trio closed 2026-07-24: I4 (borough metadata duplication — resolved by removal, `score/app.py` is the single holder), I6 (DLQ on async Lambdas — moot, all 7 functions are APIGW-synchronous), I14 (`PROJECT_DOCUMENTATION.md` — fully refreshed).
