...
- **`/aws-debug` IS BLOCKED - all four log verbs denied, re-measured 2026-09-03.** `logs:DescribeLogGroups`, `logs:DescribeLogStreams` and `logs:FilterLogEvents` each return `AccessDeniedException` naming `user/flightmap-dev`, which also puts `logs:GetLogEvents` out of reach since it needs a stream name. **This line claimed the OPPOSITE one day earlier**, on 2026-09-02, "proven by reading production output" - and the re-measurement followed that entry's own advice exactly: the LIVE group name it had resolved and recorded (`.../ScoreFunction-AQH1Sxwg3LaF`), and a match on `AccessDenied` rather than on exit status. **The cause is NOT established.** Either the policy changed between the two days or the 2 Sep measurement was mistaken; do not guess, and do not "restore" anything until it is known. What it does establish is that **a permission is a timestamp, not a property** - the 2 Sep entry stated it in the present tense with no re-check attached, so nothing could detect it going stale, and the blocking `log retention == privacy.html` gate caught it only because it happens to need the same verb. The stale-suffix trap that entry documents is still real and worth keeping: **a Lambda's log-group suffix changes whenever CloudFormation REPLACES the function**, a missing group answers `ResourceNotFoundException`, and a probe reading exit status alone turns that into a false permissions wall - so resolve names, never hardcode a suffix. (Note the sibling: the command list near the top of this file still says `/aws-debug` is "LIMITED - no log read on this account". Yesterday's correction touched one of the two lines, so **the UNCORRECTED one is now the accurate one** - mirror drift, again.) Also denied: `cloudwatch:DescribeAlarms`, `ListMetrics`/`GetMetricData`, `lambda:ListFunctions`, and `iam:ListAttachedUserPolicies` - so the live policy **cannot be read from here at all**, and there is no admin profile on this machine (`default` holds an invalid token). **`backend/iam-policy.json` declares every one of these verbs and is therefore ASPIRATIONAL, not applied.** Two loaders already say so in comments (`load_nspl.py:170`, `load_defra_raster.py:295`) and **nothing compares the file to the live policy** - a third instance of a hazard recorded in prose instead of converted to a check. This currently REDS a blocking stage, so it gates every commit in the repo until the console work lands. See `OPERATIONS.md` §6.
