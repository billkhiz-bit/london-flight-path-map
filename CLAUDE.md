...
| roadNoise / airQuality / flood bands | `build_borough_bands.py --check` | DEFRA Round 4 road Lden + DEFRA background maps + EA RoFRS. **Air quality is every city; road noise and flood are NOT - Leicester 0/8 and Teesside 0/5 carry neither** (measured from `borough-extra.json` 2026-08-21). `paintBoroughLayer()` skips what is missing, so the MAP is honest; this row was the thing over-claiming. **Since v3.9 the BANDS are still display-only but the CONTINUOUS fields beside them SCORE** - see the `environment` row below. |
| **environment** (v3.9, 2026-08-26) | `build_borough_bands.py --check` / `--sync-lambda` | **Air quality 0.65 + flood 0.35, from `airQualityWhoRatio` and `floodMediumOrHighPct` - the CONTINUOUS fields, never the three-band map summaries** (68.1% of boroughs share the modal air band; the ratio has 60 distinct values). Anchored on PUBLISHED thresholds: WHO 2021 guideline -> the UK legal limit for NO2 (40 ug/m3), and 0% -> the EA's 10% Medium-or-High cut. **A SCORING input, so it lives in BOTH holders** - `test_borough_data_parity.py` covers both fields, plus a `NAME_ALIASES` drift guard. 94/99 boroughs, 77 `measured`, 17 `partial`, 5 `unavailable` (NYC alone). |
...
| p8 | `build_progress8.py --check` | DfE KS4 2022/23 **Revised**, 0 differ; advisory preflight stage since 2026-08-24 (its gitignored KS4 bundle cannot be auto-fetched - EES 403s non-browser UAs - so blocking would gate every fresh clone) |
...
**Air quality and flood LEFT the display-only category on 2026-08-26 (v3.9).** The paragraph below is still true of the three *map layers* - the bands are drawn, not scored - but `airQualityWhoRatio` and `floodMediumOrHighPct`, the continuous fields derived alongside them, are now the `environment` component. **Road noise is the one that is still purely displayed**, and is scheduled for v4.0 with rail as a `quiet` noise composite. `plannedComponents` no longer offers `flood` or `airQuality`; it is down to `epcDistribution` and `crimeBreakdown`.
