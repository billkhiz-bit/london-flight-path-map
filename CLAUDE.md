...
newly-covered area does not keep saying otherwise. **That cache is the BROWSER's
only** (audit M1, measured 2026-09-14: `X-Cache: Miss` on every call) - API
Gateway's edge does not hold the badge, so every distinct viewer's first render
is a Lambda invocation and the 5 RPS route throttle is the one server-side
bound. The fix is a decision in ROADMAP ("What each open decision entails", item
2); a cached `/badge*` behaviour on the site's own distribution is the
recommended, free one.
