...
ONS NSPL postcode index, partition key `postcode` (spaces stripped, uppercased — the same key format as the noise-raster table, so one normalisation serves both). Populated offline by `scripts/load_nspl.py` from `data/nspl.csv`. **First full load completed 2026-07-26: 2,699,393 rows written, 24,203 skipped (unpositioned / no geography / Channel Islands + IoM), 904,453 terminated, 332,308 London, 0 spaced-form mismatches — 5.80 hours wall-clock at ~129 rows/s** (an earlier ~40 min estimate was ~10× optimistic; ~£1.50 of write units, ~£0.18/month storage with PITR). **Changed 2026-07-27:** the loader now writes via `BatchWriteItem` (25 items per request, ~96% fewer round trips), which needs `dynamodb:BatchWriteItem` — present in `backend/iam-policy.json` but **not yet applied to the live policy**. Until it is, the loader falls back automatically to the per-item path at the old speed, so a roll that still takes ~6 hours means the grant never landed. The next full load is otherwise unmeasured; do not quote a figure until a real run produces one. Holds the **February 2026** vintage — verify the edition from the data (`max(dointr)`) rather than the hardcoded `NSPL_VINTAGE` constant, and roll to the August 2026 edition when it ships. A `__META__` item in the table records vintage, row counts, source and load policy. `describe-table`'s `ItemCount` lags by ~6 hours and read 0 throughout the load, so verify with `get-item`, never `describe-table`. Attributes: `lat`/`lon` centroid, `lad` (ONS district code, every row), `b` (canonical borough name, only on the 33 London districts), `rgn` (England-only), `dt` (presence = terminated), `q` (positional quality, omitted when building-level). Absence is meaningful, so the common case is the cheapest item.
...
**Attribution is not optional.** The CSV is a derived work — every row carries an ONS NSPL centroid and a DEFRA-derived quiet score — so OGL v3.0 attribution survives into it. It appears twice on purpose: a `sources` column on every row (the copy that cannot be separated when a customer emails the CSV on its own) and a companion `<output>.sources.txt` with the full notices, the OGL link and the ONS/OS/Royal Mail copyright. Both come from the API's own `build_sources()`, and the companion file is written *after* the run so it reflects what was actually used rather than what was configured. See `LICENSING.md` → "Derived exports".
