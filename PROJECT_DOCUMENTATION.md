...
ONS NSPL postcode index, partition key `postcode` (spaces stripped, uppercased — the same key format as the noise-raster table, so one normalisation serves both). Populated offline by `scripts/load_nspl.py` from `data/nspl.csv` (2,699,393 positioned rows; **~6-7 hours measured 2026-07-26 at ~130 rows/s** — an earlier ~40 min estimate was roughly 10× optimistic — ~£1.50 of write units, ~£0.18/month storage with PITR). First load ran 2026-07-26 against the **February 2026** NSPL vintage; roll to the August 2026 edition when it ships. `describe-table`'s `ItemCount` lags by ~6 hours, so verify a load with `get-item`, never `describe-table`. Attributes: `lat`/`lon` centroid, `lad` (ONS district code, every row), `b` (canonical borough name, only on the 33 London districts), `rgn` (England-only), `dt` (presence = terminated), `q` (positional quality, omitted when building-level). Absence is meaningful, so the common case is the cheapest item.
