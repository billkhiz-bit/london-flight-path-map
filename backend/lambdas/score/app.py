...
CITY_PROVENANCE = {
    ...
    'westyorkshire': {
        ...
        'breakdown': {
            'quiet': "PROVISIONAL ESTIMATE derived from Leeds Bradford Airport (LBA) runway 14/32 alignment and its extended approach centreline. NOT sampled from the DEFRA Round 4 raster. DEFRA HAS published a Round 4 Lden surface for this airport; we have not yet sampled it, so the gap is in our pipeline and not in the coverage published by the regulator. So the dB Lden thresholds in METHODOLOGY section 3 are not evidenced here. The distance ladder is calibrated on Heathrow, which is several times this airport's size, so the bands reach further than it really does and are PESSIMISTIC rather than optimistic. Corridor waypoints are spaced ~4 km against London's ~1 km, so corridor distances are not comparable between cities. Indicative only.",
    ...
    'manchester': {
        ...
        'breakdown': {
            'quiet': 'PROVISIONAL ESTIMATE derived from Manchester Airport (MAN) runway alignment and approach geometry. NOT sampled from the DEFRA Round 4 raster. DEFRA HAS published a Round 4 Lden surface for this airport; we have not yet sampled it, so the gap is in our pipeline and not in the coverage published by the regulator. So the dB Lden thresholds in METHODOLOGY §3 are not evidenced for this city. Corridor waypoints are also spaced more coarsely than London\'s (~5 km against ~1 km), so corridor distances are not directly comparable between the two. Treat as indicative only.',
