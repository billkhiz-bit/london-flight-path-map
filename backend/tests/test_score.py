...
class PostcodeTableTests(_LocalTierFixture, unittest.TestCase):
    ...
    def test_unsourced_component_says_so(self):
        # Manchester has no schools/crime/transport/healthcare input, so its
        # liveability is a default rather than a measurement. The response has
        # to admit that; a silent placeholder reads as data.
        body, status = app.resolve_query({'city': 'manchester', 'borough': 'Trafford'})
        self.assertEqual(status, 200)
        self.assertIn('NO DATA SOURCED', body['sourceBreakdown']['live'])
        # And its noise bands are estimates, not DEFRA samples.
        self.assertIn('ESTIMATE', body['sourceBreakdown']['quiet'])
        self.assertNotIn('DEFRA Strategic Noise Mapping (Round 4, 2022). Resolution chain',
                         body['sourceBreakdown']['quiet'])
